...
- **Use streaming responses by default** — only buffer when a plugin explicitly requires it. The buffering requirement is pre-computed per-proxy in `PluginCache` for O(1) lookup at request time. For reqwest-backed responses, the streaming path uses `CoalescingBody` (128 KB target) to batch small backend chunks into larger frames, reducing write syscalls. Additionally, **adaptive response buffering** (`FERRUM_RESPONSE_BUFFER_THRESHOLD_BYTES`, default 2 MiB) collects moderate-sized bodies (256 KB – threshold) with known Content-Length into a single allocation to avoid async frame-by-frame iteration overhead. This only activates when no plugins require body buffering. See `docs/response_body_streaming.md` for the full decision flow.
...
| `FERRUM_RESPONSE_BUFFER_THRESHOLD_BYTES` | `2097152` | Responses with known Content-Length between 256 KB and this threshold are buffered instead of streamed. Reduces async iteration overhead for moderate-sized bodies. `0` = disabled (always stream). Only applies when no plugins require body buffering |
