...
- **kTLS (kernel TLS)**: `socket_opts::ktls::enable_ktls()` installs TLS session keys into the kernel via `setsockopt(SOL_TLS)` after the rustls handshake completes (Linux 4.13+). This enables `splice(2)` on TLS-encrypted TCP proxy paths, eliminating two userspace copies per chunk. Supports AES-128-GCM and AES-256-GCM cipher suites. Gated by `FERRUM_KTLS_ENABLED` (default `auto` — probes `/proc/modules` for the `tls` module at startup).
- **io_uring splice**: `socket_opts::io_uring_splice` provides infrastructure for io_uring-based splice operations (Linux 5.6+). `check_io_uring_available()` probes the kernel at startup. `splice_with_fallback()` transparently falls back to `libc::splice` when io_uring is unavailable. Gated by `FERRUM_IO_URING_SPLICE_ENABLED` (default `auto` — probes `io_uring_setup` syscall).
...
- **Connected UDP frontend sockets**: `socket_opts::create_connected_udp_socket()` creates per-client connected UDP sockets that bypass the kernel routing table lookup on `send()`. Uses `SO_REUSEADDR` + `SO_REUSEPORT` to share the listener port. Saves 5-10% CPU for concentrated traffic patterns. Enabled by default (`FERRUM_UDP_CONNECTED_SOCKETS_ENABLED=true`) — pure win for any UDP proxy with repeat clients.
...
| `FERRUM_IO_URING_SPLICE_ENABLED` | `auto` | Enable io_uring-based splice for TCP proxy zero-copy relay (Linux 5.6+). Falls back to libc splice if unavailable. Values: `auto`/`true`/`false` |
...
| `FERRUM_UDP_CONNECTED_SOCKETS_ENABLED` | `auto` | Connected UDP frontend sockets per client address (Linux). Bypasses routing table lookup on sendto. Values: `auto`/`true`/`false` |
