...
- **`MSG_ZEROCOPY`**: `socket_opts::set_so_zerocopy()` enables zero-copy send on TCP sockets (Linux 4.14+). Avoids userspace→kernel copy for large sends. Applied at the socket level via `SO_ZEROCOPY` — the kernel handles per-send thresholds internally. Gated by `FERRUM_MSG_ZEROCOPY_ENABLED` (default `auto`).
