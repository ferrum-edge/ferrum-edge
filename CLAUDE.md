...
**HTTP/3 frontend architecture**: The H3 listener in `http3/server.rs` is a standalone QUIC server that handles its own request lifecycle (plugin phases, auth, route matching). For backend communication, it extracts the request and dispatches via the same `DispatchKind`-based logic the H1/H2 handler uses. When the matched proxy is `HttpsH3Preferred` and the request flavor is `Plain`, backend bytes flow through the `Http3ConnectionPool` (quinn/h3) with `CoalescingH3Body` / `DirectH3Body` wrappers; for Grpc/WebSocket flavors or non-H3-preferred proxies, the H3 frontend falls through to reqwest / H2 pool / WebSocket upgrade just like an H1/H2 client would. This is the decoupling introduced in the BackendScheme refactor — an H3 client can hit any `https` backend regardless of whether the upstream speaks QUIC. The reverse direction (H1/H2 frontend → H3 backend) remains supported via the same `HttpsH3Preferred` dispatch kind. The H3 listener enforces its own per-header and total-header size limits (`FERRUM_MAX_SINGLE_HEADER_SIZE_BYTES` / `FERRUM_MAX_HEADER_SIZE_BYTES`) since it does not use hyper's built-in header validation — the host header extracted for routing is a substring of an already-validated header, so separate host-length validation is unnecessary.
