...
**Multi-auth**: `AuthMode::Multi` recognizes both `ctx.identified_consumer` (consumer-backed auth) and `ctx.authenticated_identity` (external JWKS/OIDC identity) as successful authentication. First-success-wins semantics apply. If every auth plugin reports missing credentials, the auth phase falls through unauthenticated rather than rejecting; anonymous access is then decided by later authorization plugins such as ACL.
