...
**`/health` DB check is intentionally cached (15s TTL)**: The `/health` and `/status` endpoints are unauthenticated (by design — load balancer probes need them). Without caching, each call runs `SELECT 1` (SQL) or `ping` (MongoDB), which an attacker with admin port access could flood to exhaust the DB connection pool (`FERRUM_DB_POOL_MAX_CONNECTIONS`, default 10), starving config polling and admin writes. The `CachedDbHealthResult` in `AdminState` caches the boolean result for 15 seconds via lock-free `ArcSwap`. Do not remove this cache or make `/health` call `db.health_check()` directly on every request. When the database is connected, the response includes `database.pool` with connection pool statistics (`size`, `idle`, `active`, `max_connections`, `min_connections`, and optional `read_replica` sub-object) via `DatabaseBackend::pool_stats()`. MongoDB returns `None` (pool managed internally by the driver).
...
│   ├── body.rs                # ProxyBody sum type (Full vs Tracked) with StreamingMetrics, CoalescingBody (reqwest), CoalescingH2Body (hyper H2), CoalescingH3Body (h3), SizeLimitedStreamingResponse
...
│   ├── udp_batch.rs           # Batched UDP recv via recvmmsg(2), batched send via sendmmsg(2) and GSO, IP(v6)_PKTINFO capture/attach
...
├── pool/                      # Shared backend pool shell (`PoolManager` + `GenericPool<M>`)
...
The Proxy struct routes differently per wire scheme. The required-ness of `hosts`/`listen_path`/`listen_port` is enforced at validation time (admin API, file loader, DB loader) and repeated here for contributor reference:

| Scheme family | `backend_scheme` values | Routing on | Field rules |
|---|---|---|---|
| HTTP-family | `http`, `https` | `hosts` + `listen_path` | At least one of `hosts` or `listen_path` MUST be set. Both may be set together. `listen_port` MUST be `None`. **`backend_scheme` is OPTIONAL on HTTP-family proxies** — defaults to `https` at load time (secure default). `backend_prefer_h3: true` opts the proxy into trying HTTP/3 to the backend first, falling back to h2/h1.1 via ALPN on failure. |
| Stream-family | `tcp`, `tcps`, `udp`, `dtls` | `listen_port` | `listen_port` MUST be set. `listen_path` MUST be `None` — a populated `listen_path` on a stream proxy is a hard error. **`backend_scheme` is REQUIRED on stream proxies** — there is no safe default for transport+TLS intent. |
...
**`backend_read_timeout_ms` / `backend_write_timeout_ms` are HTTP/gRPC-only on the TCP relay**: the `bidirectional_copy` helper accepts per-direction `backend_read_timeout` / `backend_write_timeout` arguments and the unit tests exercise them, but the production TCP code path passes `None` for both. The proxy schema defaults those fields to 30,000 ms (with a SQL CHECK constraint requiring `> 0`), which is appropriate for HTTP request/response cycles but wrong for raw TCP — `tokio::time::timeout(t, reader.read(...))` measures wall-clock time of a single read call, so a backend that goes silent during a long client upload (request/response stream protocols) would falsely trip `ReadWriteTimeout` after the 30 s default. A `tokio::time::timeout(t, writer.write_all(...))` similarly measures total flush time of one buffer and would misclassify slow-but-progressing writes as a stalled socket. Both are P1/P2 issues flagged by Codex on PR #454. The proper fix is an inactivity-based watchdog (HAProxy `timeout server` semantics — pause the timer while the other direction is making progress, reset on partial-write progress); until that lands, raw TCP relies on `tcp_idle_timeout_seconds` + the OS TCP keep-alive, and the per-direction slots stay wired through the helper for testing. The bench harness sets `FERRUM_TCP_IDLE_TIMEOUT_SECONDS=30` and `FERRUM_TCP_HALF_CLOSE_MAX_WAIT_SECONDS=30` for synthetic scenarios; production deployments should pick TCP-appropriate values.
...
1. `on_request_received` — OTel tracing (25), correlation ID (50), CORS preflight (100), request termination (125), IP restriction (150), geo restriction (175), bot detection (200), spec expose (210), SSE validation (250), gRPC-Web translation (260), gRPC method router (275), request size limiting (2800), rate limiting (2900), transaction debugger (9200)
2. `authenticate` — mTLS auth (950), JWKS auth (1000), JWT auth (1100), key auth (1200), LDAP auth (1250), basic auth (1300), HMAC auth (1400)
3. `authorize` — Access control / ACL (2000), rate limiting (2900). Supports consumer username and ACL group allow/deny lists, plus `allow_authenticated_identity` for external JWKS/OIDC identities without consumer mapping
4. `before_proxy` — OTel tracing (25), correlation ID (50), geo restriction (175), SSE request shaping (250), gRPC-Web (260), gRPC method router (275), SOAP WS-Security (1500), AI semantic cache (2700), request deduplication (2750), request size limiting (2800), GraphQL (2850), rate limiting (2900), AI prompt shield (2925), body validator (2950), AI request guard (2975), AI federation (2985), request transformer (3000), serverless function (3025), response mock (3030), gRPC deadline (3050), request mirror (3075), load testing (3080), response caching (3500), compression (4050), AI rate limiter (4200)
5. `on_final_request_body` — Post-transform request body validation. gRPC-Web (260) validates decoded gRPC frame structure. Request size limiting re-checks after request_transformer rewrites. Body validator validates gRPC protobuf requests against descriptors (unary RPCs only, with gzip decompression for compressed frames) and re-checks JSON/XML after request_transformer rewrites
6. `after_proxy` — OTel tracing (25), correlation ID (50), CORS headers (100), SSE response headers (250), gRPC-Web (260), AI semantic cache (2700), rate limiting (2900), response size limiting (3490), response caching (3500), response transformer (4000), compression (4050), AI rate limiter (4200), transaction debugger (9200). Rejects are now enforced on the response path across HTTP, HTTP/3, and gRPC
7. `on_final_response_body` — Post-transform response body hooks. Request deduplication (2750) stores responses for in-flight dedup, AI semantic cache stores responses, response size limiting, response caching (includes a cacheability predictor that maintains an LRU of known-uncacheable keys to skip cache lookups for assets that were historically uncacheable), and response-side body validator operate on the final client-visible body (after response_transformer), not the raw backend body
...
**Multi-auth**: `AuthMode::Multi` recognizes both `ctx.identified_consumer` (consumer-backed auth) and `ctx.authenticated_identity` (external JWKS/OIDC identity) as successful authentication. First-success-wins semantics apply. Missing credentials let the auth phase try the next configured auth plugin, but if the chain finishes without any authenticated identity, the request is rejected before backend dispatch.
...
**Request body buffering**: Two-tier system — config-time upper bound (`requires_request_body_buffering()`) determines if a proxy _may_ need buffering, then request-time check (`should_buffer_request_body()`) decides per-request. Only plugins that read the body (graphql, body_validator, ai_request_guard, ai_prompt_shield, ai_federation) trigger buffering; transform-only plugins do not force early prebuffering. gRPC request bodies use the same two-tier check: when no plugins need the body and no retries are configured, `proxy_grpc_request_streaming()` forwards the `Incoming` body frame-by-frame via `GrpcBody::Streaming` without collecting. When retries or plugins require the body, `proxy_grpc_request()` collects via `BodyExt::collect()` and wraps in `GrpcBody::Buffered`.
...
**PluginHttpClient TLS limitations** — plugins that bypass the normal proxy dispatch (e.g., `ai_federation` via "terminate and respond") use the shared `PluginHttpClient`, which supports only global TLS settings (`FERRUM_TLS_CA_BUNDLE_PATH`, `FERRUM_TLS_NO_VERIFY`). Per-proxy backend TLS overrides (`backend_tls_server_ca_cert_path`, `backend_tls_client_cert_path`, `backend_tls_verify_server_cert`), CRL checking, and TLS policy (cipher suites, protocol versions) do not apply. For providers behind private endpoints, add internal CAs to the global CA bundle PEM file (include public root CAs alongside internal CAs since CA exclusivity disables webpki roots when a custom bundle is set). This is an intentional design trade-off — the `PluginHttpClient` is shared across all plugins and does not have per-destination TLS differentiation. The main proxy path (`ConnectionPool`, `GrpcConnectionPool`, etc.) remains the correct choice when per-backend TLS granularity is needed.
...
- **Use streaming responses by default** — only buffer when a plugin explicitly requires it. The buffering requirement uses a two-tier check: config-time upper bound (`requires_response_body_buffering()` in `PluginCache`, O(1) lookup) then per-request refinement (`should_buffer_response_body(&RequestContext)`) so plugins can skip buffering when the request context makes it irrelevant (e.g., `compression` skips when `Accept-Encoding` is absent, AI plugins skip non-POST/non-JSON requests). For reqwest-backed responses, the streaming path uses `CoalescingBody` (128 KB target) to batch small backend chunks into larger frames, reducing write syscalls. **Small response buffering** (`FERRUM_RESPONSE_BUFFER_CUTOFF_BYTES`, default 64 KiB) eagerly buffers responses with known `Content-Length ≤ cutoff` into a single allocation — a single `bytes().await` is cheaper than the async coalescing adapter for typical JSON API payloads. SSE (`text/event-stream`) responses always stream regardless of size. When `max_response_body_size_bytes > 0` and Content-Length is absent, a `SizeLimitedStreamingResponse` adapter enforces the limit frame-by-frame without buffering the entire body. See `docs/response_body_streaming.md` for the full decision flow.
...
- Runtime flavor detection: `detect_http_flavor(&req) -> HttpFlavor` in `src/proxy/backend_dispatch.rs` classifies each request as `Plain`, `Grpc`, or `WebSocket`. One header-map lookup + a 16-byte ASCII prefix match for the gRPC path; zero allocations. Called once per request, shared between the H1/H2 handler and the H3 frontend.
- Pre-computed dispatch kind: `Proxy.dispatch_kind: DispatchKind` is populated at config-load time by `GatewayConfig::resolve_dispatch_kind()` — the hot path reads a single 1-byte enum instead of branching on `Option<BackendScheme> + bool`. Variants are `HttpPool`, `HttpsPool`, `HttpsH3Preferred`, `TcpRaw`, `TcpTls`, `UdpRaw`, `UdpDtls`.
- H3 backend dispatch: `proxy.dispatch_kind == DispatchKind::HttpsH3Preferred` (Plain flavor only — gRPC/WebSocket fall through to the HTTPS pool even on H3-preferred proxies since H3 gives them nothing).
- H2 direct pool dispatch: `matches!(proxy.dispatch_kind, HttpsPool | HttpsH3Preferred) && can_use_direct_http2_pool(...) && !state.http2_pool.is_known_http1_backend(proxy)`. The learning cache short-circuits backends that have been seen to negotiate `http/1.1`, saving a TLS handshake per request.
...
**HTTP/3 frontend architecture**: The H3 listener in `http3/server.rs` is a standalone QUIC server that handles its own request lifecycle (plugin phases, auth, route matching). Backend dispatch branches on `(proxy.dispatch_kind, http_flavor)`:

- **`HttpsH3Preferred + Plain` (native H3 fast path)** — backend bytes flow through the `Http3ConnectionPool` (quinn/h3) with `CoalescingH3Body` / `DirectH3Body` wrappers. Fully streamed both directions.
- **Everything else (cross-protocol bridge)** — delegated to `crate::http3::cross_protocol::run` in `src/http3/cross_protocol.rs`. Uses `state.connection_pool` (reqwest) for Plain flavor and `state.grpc_pool` for Grpc flavor — the same backends the H1/H2 proxy path uses — so a single `https://backend` serves H1/H2/H3 clients uniformly. WebSocket over H3 (RFC 9220 Extended CONNECT) returns 501 with an explanatory message; operators should send WS upgrades over H1/H2.

The cross-protocol bridge's buffering policy mirrors the rest of the codebase's two-tier "smart about when to buffer" logic:

- **Request body — buffered, size-bounded** into a `Vec<u8>` up to `max_request_body_size_bytes`. Matches what nginx/traefik do for cross-protocol bridges: streaming the H3 recv stream through reqwest requires a `'static` body Stream, which cannot safely capture the `&mut` borrow the H3 listener holds on the request stream. The native H3 fast path above stays fully streamed — buffering applies only when the operator has already opted into a cross-protocol hop.
- **Response body — streamed frame-by-frame with coalescing** using the same `http3_coalesce_min_bytes` / `http3_coalesce_max_bytes` / `http3_flush_interval_micros` knobs as the native H3 pool writer, so both paths produce identical QUIC frame cadence. Reqwest responses poll `chunk().await`; hyper gRPC responses poll `frame().await` so trailers separate from data for `send_trailers`. Size ceilings (`max_response_body_size_bytes`) are enforced inline in the coalesce loop so a backend cannot evade the limit by withholding `Content-Length`.
- **gRPC trailers** — mandatory `grpc-status` / `grpc-message` signalling is forwarded via H3 `send_trailers`, preserved across the H3↔H2 hop for both buffered and streaming gRPC responses.

The reverse direction (H1/H2 frontend → H3 backend) remains supported via the same `HttpsH3Preferred` dispatch kind. The H3 listener enforces its own per-header and total-header size limits (`FERRUM_MAX_SINGLE_HEADER_SIZE_BYTES` / `FERRUM_MAX_HEADER_SIZE_BYTES`) since it does not use hyper's built-in header validation — the host header extracted for routing is a substring of an already-validated header, so separate host-length validation is unnecessary.
...
**gRPC proxy architecture**: Uses hyper's HTTP/2 client directly (not reqwest) to preserve HTTP/2 trailers (`grpc-status`, `grpc-message`). The `GrpcConnectionPool` maintains sharded H2 connections with round-robin distribution. The pool uses a `GrpcBody` sum type (`Buffered(Full<Bytes>)` | `Streaming(Incoming)`) so the same `SendRequest<GrpcBody>` pool handles both buffered (retries, plugin body transforms) and streaming (zero-copy fast path) request bodies. When no plugins need the request body and no retries are configured, `proxy_grpc_request_streaming()` forwards the `Incoming` body frame-by-frame without collecting — each H2 DATA frame is forwarded immediately, with memory bounded by the H2 window size. When `stream_response=true` (no retries, no body-buffering plugins), the response `Incoming` body is wrapped in `CoalescingH2Body` which batches small HTTP/2 DATA frames into 128 KB chunks before forwarding to the client, reducing per-frame overhead. The adapter is trailer-safe: non-data frames (TRAILERS) are stashed while buffered data is flushed first, then returned on the next poll. This improved gRPC throughput at all payload sizes (up to +35% at 5MB).

**Connection pool keys**: The shared pool shell lives in `src/pool/mod.rs`, but each concrete pool still owns its key format because the key defines connection identity. The key must include every field that affects connection *identity* — destination, TLS trust, client credentials, and DNS routing. Missing a field allows two proxies with different configs to share a connection (pool poisoning); adding unnecessary fields causes fragmentation and P95 regressions. All keys use `|` as the field delimiter because `:` appears in IPv6 addresses and would create ambiguous key boundaries.
...
| gRPC | `proxy/grpc_proxy.rs` | `{host}\|{port}\|{tls}\|{dns_override}\|{ca_path}\|{mtls_cert}\|{verify}` | `#N` suffix | `tls` is bool from `matches!(proxy.backend_scheme, Some(BackendScheme::Https))`. The gRPC pool is entered at runtime when a request's content-type starts with `application/grpc` and the proxy is HTTP-family — gRPC is no longer a separate scheme. Shard key reuses a pre-allocated buffer. |
...
- **Thread-local pool key buffers** — backend pool key generation uses `thread_local!` `String` buffers reused across requests on the same tokio worker. The DashMap lookup happens while borrowing the buffer, so cache hits (99%+ of requests) allocate zero heap memory. Only the cold path (first request per unique proxy config) allocates a `String` for DashMap insertion. The shared implementation now lives in `src/pool/mod.rs`; concrete pool formats remain in `connection_pool.rs`, `proxy/http2_pool.rs`, `proxy/grpc_proxy.rs`, and `http3/client.rs`.
...
- **Don't replace reqwest with H3 pool for HTTP/3 frontend→backend**: Tested and reverted. QUIC has higher per-request overhead than TCP/H2 for small payloads (~10x regression). reqwest's HTTP/2 pooling is highly optimized and auto-negotiates the best protocol via ALPN. However, the **H1/H2 frontend → H3 backend** path now uses `Http3ConnectionPool::request_streaming()` when `stream_response=true`, returning `ResponseBody::StreamingH3` with a live `RequestStream` wrapped in `CoalescingH3Body`/`DirectH3Body` — this avoids buffering the entire H3 response into memory before forwarding to the client.
...
- **H2 response coalescing for gRPC and HTTP/2 direct paths**: `CoalescingH2Body` in `src/proxy/body.rs` batches small HTTP/2 DATA frames from hyper's `Incoming` body into 128 KB chunks before forwarding. This is the H2 equivalent of `CoalescingBody` (which wraps reqwest byte streams for HTTP/1.1). The adapter is trailer-safe: when a non-data frame (TRAILERS) arrives while the buffer has unflushed data, it stashes the trailer, flushes the buffer, and returns the stashed trailer on the next poll. This improved gRPC throughput by up to +35% at large payload sizes. Applied to both the gRPC streaming path and HTTP/2 direct pool path. **Do not replace this with raw `ProxyBody::streaming_incoming()` — it was removed because the non-coalescing path has measurably worse throughput.**
- **H3 response coalescing for HTTP/3 backend paths**: `CoalescingH3Body` in `src/proxy/body.rs` bridges h3's `recv_data()` async API to `http_body::Body`, batching small QUIC response chunks into configurable-size frames (using `h2_coalesce_target_bytes`). `DirectH3Body` provides a zero-overhead passthrough when no coalescing is needed. Both are used via `ResponseBody::StreamingH3` for H1/H2 frontend → H3 backend streaming, eliminating the previous full-body buffering on that path.
...
- **UDP IP(v6)_PKTINFO reply-source selection**: `socket_opts::set_ip_pktinfo()` / `set_ipv6_recvpktinfo()` enable pktinfo on the frontend UDP socket (Linux). On recv, `extract_pktinfo_local_addr()` parses `IP_PKTINFO` / `IPV6_PKTINFO` cmsg into `PktinfoLocal { ip, ifindex }` and `RecvMmsgBatch` surfaces it per-slot. The first datagram that exposes pktinfo pins the value onto `UdpSession.local_addr` via `OnceLock` (first-capture-wins; subsequent datagrams reuse lock-free). On reply send, the captured IP + ifindex is attached as `IP(v6)_PKTINFO` ancillary data combined with `UDP_SEGMENT` in a single `sendmsg(2)` via `socket_opts::send_with_pktinfo()`, saving one kernel routing-table lookup per GSO flush (~2% at 100K+ datagrams/sec on hosts with large routing tables). Correctly preserves the receive-side zone for scoped IPv6 (link-local `fe80::/10`) by carrying `ipi6_ifindex`. Address family mismatches between captured source and destination are silently dropped (slot queued without pktinfo). Gated by `FERRUM_UDP_PKTINFO_ENABLED` (default `auto` — probes both v4 `IP_PKTINFO` and v6 `IPV6_RECVPKTINFO` on transient sockets; activates if either succeeds). **Enabling pktinfo also flips the recv loop to `readable() + recvmmsg` primary wakeup** so the first datagram of each wakeup carries cmsg; one-shot UDP flows (e.g. DNS) get correct reply-source selection on the initial packet.
...
- **Supported databases**: PostgreSQL, MySQL, SQLite (via sqlx), MongoDB (via `mongodb` crate). SQLite connections are configured with `PRAGMA journal_mode=WAL` (concurrent reads/writes), `PRAGMA busy_timeout=5000` (5s retry on lock contention), and `PRAGMA foreign_keys=ON` — all set via `after_connect` on every pool connection.
- **Database backend abstraction**: The `DatabaseBackend` trait in `src/config/db_backend.rs` abstracts all database operations. `DatabaseStore` (sqlx) and `MongoStore` (mongodb) both implement this trait. The admin API and operating modes use `Arc<dyn DatabaseBackend>` for polymorphic dispatch. The trait includes `pool_stats()` which returns `DbPoolStats` (size, idle, active, max/min connections, optional read replica stats) — exposed on the admin `/status` endpoint under `database.pool`.
...
- **Migrations**: Core gateway migrations in `src/config/migrations/` (SQL: V001 initial schema with indexes, CHECK constraints, and compound query indexes). For MongoDB, `run_migrations()` creates indexes instead. Custom plugin migrations declared via `plugin_migrations()` in plugin files and tracked in `_ferrum_plugin_migrations`. Both run via `FERRUM_MODE=migrate`.
...
| `FERRUM_RESPONSE_BUFFER_CUTOFF_BYTES` | `65536` | Responses with known Content-Length ≤ this cutoff are eagerly buffered into a single allocation instead of streamed via the async coalescing adapter. For small JSON API payloads a single `bytes().await` is cheaper. SSE (`text/event-stream`) always streams regardless. `0` = disabled (always stream). Only applies when no plugins require body buffering |
...
| `FERRUM_UDP_PKTINFO_ENABLED` | `auto` | Enable `IP_PKTINFO` / `IPV6_PKTINFO` on frontend UDP sockets (Linux). Captures per-datagram local destination address + interface index on recv, reuses them as reply source on send — skips one routing lookup per `sendmsg` (combined with `UDP_SEGMENT`/GSO in one cmsg) and ensures replies exit the same interface the client targeted (important for multi-homed / NAT / scoped IPv6 link-local). Switches the recv loop to `readable()+recvmmsg` so the first datagram of each wakeup also carries cmsg. Values: `auto` (probe both v4/v6 setsockopt), `true`, `false` |
