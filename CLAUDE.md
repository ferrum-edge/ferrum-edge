...
**`backend_read_timeout_ms` / `backend_write_timeout_ms` per-direction inactivity on TCP relays**: Both fields are enforced on TCP proxy `bidirectional_copy` paths as inactivity-based watchdogs. Per-direction watermarks (`Arc<AtomicU64>`) are refreshed on every successful read (b2c) or partial write progress (c2b). The Phase 1 `select!` loop polls watermarks once per second alongside the existing `last_activity` idle check. When a watermark's staleness exceeds the configured threshold, the session is terminated with `ReadWriteTimeout` / `IdleTimeout` disconnect cause, attributed to the correct direction (`BackendToClient` for read, `ClientToBackend` for write). The chunked write loop in `copy_one_direction` replaces `write_all` with a manual `write()` loop that refreshes the write watermark on each partial progress, fixing the P2 issue (slow-but-progressing backends are not misclassified as stalled). The schema allows `0` to disable per-direction enforcement for long-lived TCP workloads (database keep-alives, SSH, IMAP). Default remains 30,000 ms. Splice, kTLS-splice, and io_uring splice paths have no per-read hook and continue to rely solely on `tcp_idle_timeout_seconds`. The bench harness sets `FERRUM_TCP_IDLE_TIMEOUT_SECONDS=30` and `FERRUM_TCP_HALF_CLOSE_MAX_WAIT_SECONDS=30` for synthetic scenarios; production deployments should pick TCP-appropriate values.
