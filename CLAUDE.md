...
**Single-protocol-per-request contract**: Within a single request, retries stay on the protocol the request started with — H3 retries hit the H3 pool, reqwest retries hit reqwest. Capability downgrade affects the NEXT request, never this one. Cross-protocol fallback within a request would bypass `proxy.retry.retry_on_methods` and risks duplicating non-idempotent requests; the failed transport attempt may have flushed headers/body to the backend before the error surfaced. The retry loop in `proxy_to_backend_inner` captures `request_was_h3` once before the loop and uses it for every iteration, so `mark_h3_unsupported` firing mid-loop only changes the dispatch decision for subsequent requests. This matches industry practice (Envoy, NGINX, HAProxy, Cloudflare, GFE): cross-protocol fallback happens at connection establishment (happy-eyeballs), not mid-request.
