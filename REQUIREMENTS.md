...
*   **Required Plugin Implementations:**
    ...
    *   **`http_logging`:** Buffers transaction summaries and sends them as a JSON array via HTTP POST to a configured URL using a background task. Config: `endpoint_url` (String), `custom_headers` (Optional Object — key-value pairs of HTTP headers to include on every batch request, supporting service-specific auth like `DD-API-KEY` for Datadog, `Api-Key` for New Relic, `X-Sumo-Category` for Sumo Logic), `authorization_header` (Optional String — **deprecated**, use `custom_headers` instead), `batch_size` (Integer, default 50), `flush_interval_ms` (Integer, default 1000), `max_retries` (Integer, default 3), `retry_delay_ms` (Integer, default 1000), `buffer_capacity` (Integer, default 10000). Batches are discarded after retries are exhausted to bound memory usage.
