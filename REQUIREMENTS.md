...
*   **DNS Caching:**
    *   `FERRUM_DNS_TTL_OVERRIDE_SECONDS`: (Optional integer seconds) - Default: disabled. When set, forces all DNS records to use this fixed TTL. By default, the cache respects each record's native TTL.
    *   `FERRUM_DNS_MIN_TTL_SECONDS`: (Integer seconds) - Default: `5`. Minimum TTL floor to prevent 0-TTL abuse.
    *   `FERRUM_DNS_FAILED_RETRY_INTERVAL_SECONDS`: (Integer seconds) - Default: `10`. Background retry interval for failed DNS lookups. `0` = disabled.
...
*   **DNS Resolution & Caching:**
    ...
    *   Cached entries MUST expire based on each record's native DNS TTL by default. A global TTL override is available via `FERRUM_DNS_TTL_OVERRIDE_SECONDS` (disabled by default). Per-proxy TTL overrides via `Proxy.dns_cache_ttl_seconds` take highest precedence. A minimum TTL floor (`FERRUM_DNS_MIN_TTL_SECONDS`, default 5s) prevents 0-TTL abuse. Upon cache miss or expiry, perform a fresh asynchronous DNS lookup.
...
*   **7.2. `Consumer` Resource:**
    ...
    *   `credentials`: `std::collections::HashMap<String, serde_json::Value>` (Keys: `jwt`, `keyauth`, `basicauth`, `hmac_auth`, `mtls_auth`. Values: JSON objects containing credential details. **Secrets MUST be stored securely hashed**).
    *   `created_at`: `chrono::DateTime<Utc>`.
    *   `updated_at`: `chrono::DateTime<Utc>`.
...
*   **Multi-Authentication Mode (`Proxy.auth_mode = "multi"`):**
    *   If a Proxy is configured for `multi` auth, execute *all* attached authentication plugins (JWKS, JWT, KeyAuth, BasicAuth) sequentially for a given request.
...
*   **Required Plugin Implementations:**
    ...
    *   **`http_logging`:** Buffers transaction summaries and sends them as a JSON array via HTTP POST to a configured URL using a background task. Config: `endpoint_url` (String), `custom_headers` (Optional Object — key-value pairs of HTTP headers to include on every batch request, supporting service-specific auth like `DD-API-KEY` for Datadog, `Api-Key` for New Relic, `X-Sumo-Category` for Sumo Logic), `batch_size` (Integer, default 50), `flush_interval_ms` (Integer, default 1000), `max_retries` (Integer, default 3), `retry_delay_ms` (Integer, default 1000), `buffer_capacity` (Integer, default 10000). Batches are discarded after retries are exhausted to bound memory usage.
    *   **`tcp_logging`:** Buffers transaction summaries and sends them as newline-delimited JSON (NDJSON) over a persistent TCP or TCP+TLS connection to a configured host:port. Config: `host` (String, required), `port` (Integer, required, 1–65535), `tls` (Boolean, default false), `tls_server_name` (Optional String), `batch_size` (Integer, default 50), `flush_interval_ms` (Integer, default 1000), `max_retries` (Integer, default 3), `retry_delay_ms` (Integer, default 1000), `buffer_capacity` (Integer, default 10000), `connect_timeout_ms` (Integer, default 5000). Connection is persistent and auto-reconnects on failure. TLS uses gateway-level CA bundle and skip-verify settings.
        *   **Transaction Summary Fields (for all logging plugins):** Timestamp Received, Client IP Address, Identified Consumer Username/ID (or null), HTTP Method, Request Path, Matched Proxy ID/Name, Backend Target URL (protocol://host:port/path - *excluding query parameters*), Final HTTP Response Status Code, Latency-Total (ms), Latency-GatewayProcessing (ms), Latency-BackendTTFB (ms), Latency-BackendTotal (ms, -1.0 for streaming responses where total is unknown at log time), Request User-Agent Header, Response Streamed (bool), Client Disconnected (bool).
    ...
    *   **`jwks_auth`:** Authenticates Bearer JWTs using one or more Identity Provider JWKS endpoints. Config: `providers` (array of IdP configs with `jwks_uri` or `discovery_url`, `issuer`, `audience`, `required_scopes`, `required_roles`, `scope_claim`, `role_claim`, `consumer_identity_claim`, `consumer_header_claim` — all per-provider fields are optional overrides of the global defaults), global `scope_claim`, `role_claim`, `consumer_identity_claim`, `consumer_header_claim`, `jwks_refresh_interval_secs`. Per-provider `consumer_identity_claim`/`consumer_header_claim` allow different IdPs to use different JWT claims for identity (e.g., Google uses `email`, Keycloak uses `preferred_username`). Supports claim-based authorization and consumer-optional identity flow. Integrates with `multi` auth mode.
    ...
    *   **`ip_restriction`:** Enforces client IP address / CIDR allow and deny rules independently of consumer authentication. Config: `allow` (list of IPs/CIDRs), `deny` (list of IPs/CIDRs).
