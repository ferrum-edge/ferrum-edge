...
**`default_max_tokens` injection note:** `default_max_tokens` keeps a top-level fallback (`max_tokens`) when the body carries a top-level OpenAI-family prompt field (`messages`, `prompt`, or `input`) — the marker an OpenAI/Anthropic/Cohere upstream reads — or carries no provider-native container at all, unless a recognized top-level cap (`max_tokens`, `max_completion_tokens`, `max_output_tokens`, or another supported top-level alias) is already present. This defeats the spoof where a client appends a provider-native marker to an OpenAI-shaped body to route the default into a field the real upstream ignores. A body that is **unambiguously provider-native** — a native container (`generationConfig`/`contents` for Gemini, `inferenceConfig` for Bedrock Converse, `inputs` for TGI) with *no* top-level OpenAI prompt field — is left to its native cap only, so a strict provider backend is not handed an unsupported top-level `max_tokens`: `generationConfig.maxOutputTokens` for Gemini/Vertex, `inferenceConfig.maxTokens` for Bedrock Converse, and `max_new_tokens` for TGI/HuggingFace. (Bedrock Converse is the irreducible exception — it shares `messages` with OpenAI, so a `messages`-bearing Bedrock body is treated as top-level-capable and fail-closed-capped.) A Bedrock body that omits `inferenceConfig` — or an Amazon Titan body (`textGenerationConfig.maxTokenCount`) — routes to the top-level target. Injection is suppressed only for fields that already cap their corresponding target, so a Responses request that already sets `max_output_tokens` does not receive a redundant `max_tokens`. `enforce_max_tokens` (reject/clamp) still reads `inferenceConfig.maxTokens` and `textGenerationConfig.maxTokenCount` when they are present.
