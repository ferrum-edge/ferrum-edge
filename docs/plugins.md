...
Request buffering is only enabled for matching JSON `POST` requests when at least one guard or transform rule is configured. Model-policy fail-closed checks apply after the plugin can inspect a well-formed buffered JSON body; empty or missing bodies, malformed JSON, and non-JSON content continue to the backend for compatibility. The realistic residual risk is **content-type confusion** — a client sends an uninspectable body (e.g. a JSON payload labelled `text/plain` or with no body at all) to a backend that parses it as JSON regardless of `Content-Type`, bypassing model policy. If the backend must never receive un-inspected requests, pair this plugin with a hard boundary: use `body_validator` (or `request_size_limiting`) to restrict `content_types` to the JSON variants you inspect and to require a non-empty body, so anything this plugin can't parse is rejected before dispatch rather than passed through.
