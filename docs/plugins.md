...
| `fail_on_missing_model` | Boolean | `true` | Reject JSON POST requests that lack a top-level string `model` field. Set to `false` only to explicitly pass such requests through to the normal backend |
...
Use this only when the normal backend has equivalent authentication, model allow-listing, rate limits, prompt/body validation, and logging. Otherwise a request with no `model` or an unsupported `model` can avoid the federated provider path entirely.
