...
The gateway plugins **cannot** turn this into a hard boundary on their own. `body_validator` returns `Continue` whenever the body is absent/empty or the `Content-Type` falls outside its `content_types` set (it validates matching bodies, it does not reject unlisted ones or require a body to be present), and `request_size_limiting` only enforces a `max_bytes` ceiling — neither offers a content-type whitelist-reject or a "body required" check. A true *"the backend never receives un-inspected traffic"* guarantee for non-JSON / no-`Content-Type` requests has to come from ensuring `ai_request_guard` sits on the **only** ingress path to the backend and/or enforcing the constraint at the network or deployment layer (e.g. restricting the backend to accept only requests that traversed the gateway).
