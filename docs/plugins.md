...
- `google_gemini` — OpenAI Chat Completions streaming requests are translated to Gemini `streamGenerateContent` (native Generative Language API and Vertex-shaped `GenerateContentResponse` streams). The endpoint must embed a `{model}` path placeholder and target `streamGenerateContent`; `alt=sse` is required and injected when absent so framing stays progressive. Auth is `x-goog-api-key`. Request translation covers system → `systemInstruction`, user/model turns, modern tool-call / tool-result history (and legacy `function_call` / `role: "function"`), `tools` → `functionDeclarations`, `tool_choice` (`none`/`auto`/`required`/named → `functionCallingConfig`), and `generationConfig` sampling fields. The model stays in the URL path (not the body). A `ResponseStreamInspector` normalizes Gemini/Vertex SSE into OpenAI `chat.completion.chunk` SSE: text parts → `delta.content`, `functionCall` → `delta.tool_calls`, `finishReason` / `promptFeedback.blockReason` → `finish_reason` (including `content_filter` for safety blocks), `usageMetadata` → a terminal usage chunk, then `data: [DONE]`. Successful termination requires a terminal `finishReason` or a prompt-level safety block (or an explicit provider `error` object); premature EOF and malformed/ambiguous frames fail closed with a fixed-cardinality `upstream_error` SSE frame (provider error bodies are never echoed). When the caller constrained the turn with `tool_choice: "none"`, any provider `functionCall` fails closed. The same byte/event/depth/output bounds as Anthropic apply before UTF-8 conversion, JSON parse, or transcode.

**Header rewriting.** Client `Authorization`, `Proxy-Authorization`, `Cookie`, `x-api-key`, `api-key`, `x-goog-api-key`, and `anthropic-version` headers are stripped before forwarding (unlike `ai_federation`, this path reuses the client's header map, so session-bearing headers must be dropped explicitly), and the provider's credential is injected (`Authorization: Bearer …` for OpenAI-compatible, `x-api-key` for Anthropic, `x-goog-api-key` for Gemini). `Host`, `Content-Type: application/json`, and `Accept: text/event-stream` are set for the provider. When Anthropic or Gemini SSE will be normalized, every client `Accept-Encoding` variant is replaced with `Accept-Encoding: identity` so the provider does not return gzip/br octets that the line-delimited normalizer would misread. Because normalization rewrites the client-visible representation even when the provider sends identity bytes, representation headers (`Content-Encoding`, `Content-Length`, content-bound validators, and `Vary: Accept-Encoding`) are always repaired. If a provider still returns a non-identity `Content-Encoding`, supported `gzip` / `br` bodies are decoded before normalization; unsupported, layered, or ambiguous case-variant codings are rejected with an OpenAI-shaped `502`. Residual encoded streaming responses use a bounded fallback (16 MiB encoded, 8 MiB decoded) and are decoded only after the complete coding and checksum can be validated; ordinary identity SSE remains progressive. Gateway-asserted consumer-identity headers (`x-consumer-username` / `x-consumer-custom-id`) are also **suppressed** for claimed requests via the shared `suppress_backend_consumer_identity_headers` metadata marker, so an authenticated consumer's internal identifiers never reach the third-party provider (the resolved principal still drives rate limiting, logging, and policy plugins).
...
- **Usage accounting depends on the provider emitting usage in the stream.** OpenAI-compatible providers emit a final usage event only when `stream_options.include_usage` is set (hence `inject_usage_options`); Anthropic usage is derived from `message_start`/`message_delta`; Gemini/Vertex usage is derived from `usageMetadata`. Providers that omit usage yield no token counts.
- Anthropic and Gemini request translation is text-first for message content in this MVP (top-level `tools`/`tool_choice` and OpenAI tool-call / tool-result history — including legacy `function_call` / `role: "function"` — are translated; non-text message-content parts are dropped). Multimodal content parts remain unsupported on the stream-router path. Mixing modern `tool_calls`/`role: "tool"` with legacy `function_call`/`role: "function"` in one request, and other unrepresentable tool-history states, fail closed with field-specific diagnostics that do not echo argument payloads.
