...
| Azure OpenAI "On Your Data" | `messages[]` (chat completions shape) | `messages[].content` and other chat-completions fields, plus `data_sources[].parameters.role_information` | `messages[].role: "system"|"developer"`, aliases, and any non-empty `data_sources[].parameters.role_information` | `max_tokens`, `max_completion_tokens` |
