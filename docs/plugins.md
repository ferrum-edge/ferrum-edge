...
- **Cache status header**: Responses include an `X-Ai-Cache-Status` header: `HIT` when the response is served from cache, `MISS` when the response is fetched from the backend and stored. Successful cache-hit responses are passed through the normal buffered response-side hooks before reaching the client, so response-side `ai_semantic_firewall`, `ai_response_guard`, response body transforms, and final-response hooks still apply to cached LLM bodies.
