...
**The AWS event-stream framing is verified, not assumed.** `application/vnd.amazon.eventstream` is HTTP *body* framing that no HTTP transport validates, so both of its documented CRC-32 checksums are checked here: the prelude checksum over the two declared length fields (verified *before* the declared length is honored, so a forged length can never drive reassembly) and the whole-message checksum over everything preceding it. The bounded header block is then walked in full — all documented AWS header value encodings (type codes 0 through 9: `BOOL_TRUE`, `BOOL_FALSE`, `BYTE`, `SHORT`, `INTEGER`, `LONG`, `BYTE_ARRAY`, `STRING`, `TIMESTAMP`, `UUID`) are traversed, and a zero-length or non-UTF-8 header name, an undefined value type, a length that runs past the block, a reserved header encoded with a non-`STRING` value type, or a repeated reserved header all fail the scan closed. A wrong-typed reserved header is *not* treated as absent: its value is not the kind name the scanner must compare against, so ignoring it would let a partial declaration fall through to the compatibility path below, and a later valid duplicate behind it cannot rescue it either.
...
The AWS event stream applies the same invariant to the payload of any frame its headers *allow* to carry usage. Such a payload is a usage candidate, so a payload (or a *successfully decoded* base64 `bytes` envelope's inner document) that is empty or is not a decodable JSON object, and a usage container whose fields cannot be read as counts, all mark the stream damaged — invalidating an older snapshot for settlement until a later valid authoritative cumulative record recovers it. Ordinary provider content is distinguished from a malformed candidate: a `chunk` frame carrying model text and no usage container at all is the normal `InvokeModelWithResponseStream` shape and is never damage, while a `metadata` frame that reported no readable usage is, because that event kind exists to report it. Frames the headers already excluded (exception, error, content, unknown, and incomplete reserved-header declarations) are not candidates and never damage the scan. CRC, prelude, header-block, and oversize failures remain hard malformed/halted rather than ordering-sensitive damage, and so does every `bytes`-envelope failure that leaves nothing to decode. The envelope is all-or-nothing: a payload that simply does **not** declare `bytes` is ordinary content (that is the normal `ConverseStream` and bare-JSON shape), while a `bytes` that is explicitly present but not a string (`null`, a number, a boolean, an object, an array), one whose encoded value exceeds the retained message bound, and one that is not decodable base64 are each an invalid envelope declaration whose real payload cannot be reconstructed. Those halt the scan, so no later frame is parsed and no later record can recover them.
...
**Aggregate accounting state is bounded, not just per-stream state.** A bounded per-stream scanner is not by itself a bound on the process: each live SSE scanner may retain up to 64 KiB and each live AWS event-stream scanner up to 256 KiB, and nothing about request scoping stops a client from opening many concurrent streams and multiplying those windows. The gateway therefore holds one process-wide budget of retained accounting bytes, shared by every `ai_rate_limiter` instance and configured with [`FERRUM_AI_STREAM_ACCOUNTING_MAX_BYTES`](configuration.md) (default 64 MiB, i.e. about 1024 concurrent SSE scanners or 256 AWS event-stream scanners). A reservation is taken *before* a scanner is constructed and released when it drops — on clean EOF, on a streaming error, on client disconnect, on deadline, on downstream termination, on plugin rejection, and on task cancellation alike.

What it bounds is exactly that: the *retained scanner state* — the SSE line window and the AWS event-stream message-reassembly window, the only allocations a scanner grows with the stream. It is not a bound on transient JSON parsing working memory for a single decoded frame or request body, which is governed by the ordinary request/response body size limits instead.

Admission happens as early as the client's own declaration allows. When the request declares a streamed response — a body-level `stream: true`, a provider-native streaming operation in the request target (`streamGenerateContent`, `invoke-with-response-stream`, `converse-stream`), or an event-stream `Accept` (either the `text/event-stream` family or AWS Bedrock's `application/vnd.amazon.eventstream`, which also selects the larger retention class) — the reservation is taken in `before_proxy`, **before the backend is dialed**. A refusal there is a gateway-local `503` with a fixed body that names no key, identity, provider, backend, configured value, or response byte; no upstream is contacted, so it produces no circuit-breaker sample, no passive-health observation, and no adaptive-concurrency measurement, and a saturated accounting budget can never present as a sick backend. It is also taken before the token reservation, so a refused stream never strands a token reservation that only window/TTL expiry would reclaim.

A compressed request body that a co-located `compression` plugin decodes is classified in `on_final_request_body`, not `before_proxy`, so its admission is taken there instead — over the decoded top-level `stream` flag plus the same path and `Accept` signals. That hook still runs **before the backend request is sent**, so such a request is refused with the same gateway-local `503` rather than degrading to a committed stream, and gzipping a request is not a way around the aggregate bound. The two admission points share one per-instance reservation, so a request that declares streaming in both places is never charged twice, and a body that does not parse as an AI request — or an AI request that asks for no streamed response — reserves nothing there.

A request that declares none of those signals anywhere but whose backend streams anyway gets a second and final admission attempt when the response representation becomes known. **At that point the refusal cannot be a rejection**: response headers and bytes are already committed, so the gateway fails closed the only way still available — no scanner is allocated, so the aggregate bound holds, and the stream settles through the configured `on_unmetered_response` posture with a `stream_accounting_capacity` detail rather than charging zero. Concretely, that means `reject` on such a stream keeps the reservation charged and warns instead of substituting a 502, exactly as it does for a stream that reported no usage.

> **Committed streams and a zero pre-reservation estimate.** These two limitations compose, and the composition is the one an operator most needs to understand. In `count_mode: "completion_tokens"` a request that omits every output cap has an estimate of `0`, so nothing is pre-reserved; if that request is also streamed, then a terminal state with no authoritative usage (no usage event, damaged framing, disconnect, or a refused accounting admission) leaves `reject` with nothing to do — it cannot replace bytes already on the wire, and "keep the reservation charged" charges zero. The stream is still settled exactly once and still recorded with its unmetered detail, but *no tokens are charged for it*. Aggregate admission does not change this: it bounds gateway memory, not the caller's budget. If streamed requests must always cost something, use `count_mode: "total_tokens"` or `"prompt_tokens"` (which always yield a non-zero prompt estimate), and/or require an output cap upstream via `ai_request_guard`'s `default_max_tokens`.
...
> **Compressed request accounting follows the normalized representation where policy is still possible.** With a co-located `compression` plugin using `decompress_request: true`, supported gzip/Brotli uploads are bounded and decoded in the request-body transform stage, after `ai_rate_limiter`'s `before_proxy` token-budget decision. The limiter therefore cannot compute a token estimate or token pre-reservation from that later plaintext; `before_proxy` performs only the plain budget check. The final request-body hook classifies the decoded body that will reach the backend and, when it declares a streamed response through its top-level `stream` flag, provider path, or `Accept`, reserves the bounded stream-scanner state before backend dispatch. Post-response reconciliation charges provider-reported usage. Without a matching decoder, or for an unsupported coding, the limiter never estimates from compressed wire bytes and treats a JSON-content-type `POST` as a fail-closed AI candidate for `on_unmetered_response`, so an uninspectable compressed AI call cannot bypass `reject`/`charge_estimate` enforcement. The limiter never decompresses bodies itself.
