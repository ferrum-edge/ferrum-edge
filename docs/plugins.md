...
| `api_chargeback_sink` JSONEachRow insert body (live batches) | the delivery worker | a provisional ceiling reservation taken before serialization from an escaping/framing bound, then shrunk to the exact retained buffer capacity | the request finishes |
