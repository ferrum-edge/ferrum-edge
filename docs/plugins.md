...
| `default_max_tokens` | Integer | *(none)* | Inject a default output-token limit if no supported token field is present; keeps a top-level fallback and also uses provider-native containers for Gemini/Bedrock/TGI-shaped bodies |
...
**`default_max_tokens` injection note:** `default_max_tokens` always keeps a top-level fallback (`max_tokens`) unless a recognized top-level cap (`max_tokens`, `max_completion_tokens`, `max_output_tokens`, or another supported top-level alias) is already present. This prevents request-controlled provider-native markers from routing the default only into a field ignored by a top-level-compatible upstream. When the body also matches a provider-native shape, the guard additionally injects that provider's native cap: `generationConfig.maxOutputTokens` for Gemini/Vertex markers, `inferenceConfig.maxTokens` when a Bedrock Converse body carries `inferenceConfig`, and `max_new_tokens` for TGI/HuggingFace bodies recognized by top-level `inputs`. A Bedrock body that omits `inferenceConfig` — or an Amazon Titan body (`textGenerationConfig.maxTokenCount`) — still cannot be disambiguated from top-level-compatible schemas for default injection. Injection is suppressed only for fields that already cap their corresponding target, so a Responses request that already sets `max_output_tokens` does not receive a redundant `max_tokens`. `enforce_max_tokens` (reject/clamp) still reads `inferenceConfig.maxTokens` and `textGenerationConfig.maxTokenCount` when they are present.
