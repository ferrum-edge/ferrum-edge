...
- Streamed non-buffered responses, including `text/event-stream`, keep the in-flight marker while the stream is active and after the stream reaches a terminal success/error/disconnect state. Because these responses do not retain a replayable body or tombstone, duplicates remain blocked until `inflight_ttl_seconds` instead of immediately re-running the backend operation
- Stale in-flight markers (request died after `before_proxy` but before `on_final_response_body` — e.g., backend timeout, downstream plugin reject, process crash — or non-buffered streams that cannot produce a replayable response) are treated as fresh after `inflight_ttl_seconds` so duplicates aren't blocked indefinitely. Tune `inflight_ttl_seconds` to cover your longest legitimate backend request; setting it too low risks duplicate side-effecting executions for slow-but-alive requests
