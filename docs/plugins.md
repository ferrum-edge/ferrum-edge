...
| `max_entry_size_bytes` | u64 | `1048576` | Maximum retained size of one completed response entry. Oversized responses are returned normally, clear the in-flight marker, and are not retained locally or serialized for Redis |
| `max_total_size_bytes` | u64 | `104857600` | Exact maximum retained size across local completed response entries. Responses that would exceed this cap are returned normally, clear the in-flight marker, and are not retained |
...
- Completed response storage is bounded by `max_entry_size_bytes` and `max_total_size_bytes`. Size-skipped responses still return to the original client, but no replayable response is retained and the in-flight marker is cleared so a later retry can execute normally
