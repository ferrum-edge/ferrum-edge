...
| `streaming.max_inspections` | uint | `64` | `inspect` only. Safety cap on provider inspections per response. Once reached, further inspectable windows honor `on_error`: `reject` cuts (fail closed), `warn`/`allow` forward best-effort |
