...
| `forward_body` | bool | `false` | Include the lossless buffered request body for every method. `body` is the authoritative exact representation: UTF-8 bytes are a string with `body_encoding: "utf8"`, and other bytes are base64 with `body_encoding: "base64"` |
...
When `forward_body` is enabled, the plugin buffers before `before_proxy` for every HTTP method and fails closed without calling the function if the exact body bytes are unavailable or the active request headers carry a non-identity `Content-Encoding`. The body is never parsed before function egress: JSON duplicate keys, whitespace, and lexical number forms therefore remain byte-for-byte intact in the authoritative UTF-8 string, while non-UTF-8 bytes retain the existing base64 representation. Empty bodies are explicit empty UTF-8 strings. The gateway's configured request-body limit bounds the retained body, and the envelope holds only this one authoritative body representation. Query/body representation ambiguity is governed input and remains fail-closed even with `on_error: "continue"`; that option applies to invocation/response failures, not to inputs the configured policy cannot inspect faithfully. A serverless body-egress instance cannot share a protocol chain with a request-body transformer; candidate admission and cache construction reject that composition so the external decision and backend cannot observe different representations. Registered custom plugins participate in the same validation through `egresses_request_body_before_finalization()` rather than a built-in-name exception.
...
`body` is authoritative and lossless rather than a parsed structured view. For a non-UTF-8 body the final two fields are instead `"body": "<base64>"` and `"body_encoding": "base64"`. A function that wants structured JSON must parse the `utf8` string after applying its policy to that exact representation.
