...
| Cross-cluster protocol parity — **remaining**: gRPC, WebSocket, raw TCP/UDP (not materialized today). **Multi-port cross-cluster now supported** via the per-port `p<port>.<fqdn>` SNI alias scheme (#2010 phase 3). | [#2010](https://github.com/ferrum-edge/ferrum-edge/issues/2010) | `docs/mesh.md` cross-cluster egress sections + "Multi-Port Cross-Cluster SNI Aliases" + matrix notes [2][3][5][7] |
