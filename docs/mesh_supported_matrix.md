...
| Cross-cluster protocol parity — **remaining**: multi-port (single-port-per-SNI), raw TCP/UDP (not materialized today), and Ambient cross-cluster **gRPC** (fail-closed — HBONE has no HTTP/2 trailer path). **Now supported**: Sidecar cross-cluster gRPC + WebSocket over east-west mesh-mTLS (#2010 phase 1–2) and Ambient cross-cluster WebSocket over the HBONE byte tunnel. | [#2010](https://github.com/ferrum-edge/ferrum-edge/issues/2010) | `docs/mesh.md` cross-cluster egress sections + matrix notes [2][3][5][7] |
