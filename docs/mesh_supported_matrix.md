...
|---|---|---|
| **GA** (equivalent to `docs/mesh.md` "Stable") | Production-suitable; exercised end-to-end against a live data path. A product promise. | **Prescriptive, at both layers.** Listed in `tests/conformance/ga_contract.yaml` and tagged `Maturity::Ga`; a semantic regression to anything but `Supported` fails CI (`tests/conformance/ga_scope.rs`). The **live** half is equally blocking: the `mesh-e2e-sidecar` and `multicluster-federation` suites run on every relevant PR and every main push, the results are merge-blocking required checks (the dedicated workflows' `Mesh E2E Sidecar Live` and `Multicluster Federation Live` gate jobs, required via branch protection), and `release.yml` refuses to ship a tag whose SHA lacks green live runs for both suites. The two suites enforce the contract differently. `mesh-e2e-sidecar` validates its emitted `live-assertions.json` in-workflow (`tests/conformance/live_contract.rs::live_contract_artifact_gate` — required IDs present + passed for the exact suite/profile/commit, no duplicate or stale artifacts). `multicluster-federation` runs **no** in-workflow artifact validator: the trusted Cross build policy freezes that workflow's Cross-sensitive surfaces, so its required-assertion gate is the fixture's own fail-closed `ferrum_live_assertions_require_all_passed` (a missing, failed, or skipped required assertion fails the live job), and the binding of that required set to the enforced, non-`live_deferred` contract rows is pinned by the hosted conformance suite (`live_contract.rs`, `mesh_multicluster_federation.rs`) in the `Tests` aggregate. |
