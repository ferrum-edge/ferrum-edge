...
| Cross-cluster protocol parity — **remaining**: multi-port (single-port-per-SNI), raw TCP/UDP (not materialized today), and Ambient cross-cluster gRPC/WebSocket (fail-closed). **Sidecar cross-cluster gRPC and WebSocket over east-west mesh-mTLS now supported** (#2010 phase 1–2). | [#2010](https://github.com/ferrum-edge/ferrum-edge/issues/2010) | `docs/mesh.md` cross-cluster egress sections + matrix notes [2][3][5][7] |
