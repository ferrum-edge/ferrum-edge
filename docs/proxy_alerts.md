...
The constructor is the admission source of truth. `ProxyAlertsConfig` in `openapi.yaml` encodes the closed objects, integer ranges (including the uint32 ceiling on `max_concurrent_dispatches`), nonempty URL / `*_env` strings, HTTP(S) URL shape with no userinfo, case-insensitive webhook methods, SMTP credential pairing, nonempty email templates, and ASCII addr-spec pattern. The following remain constructor-only because standard JSON Schema cannot express them:

- `delivery_retry_max_ms >= delivery_retry_base_ms`
- Balanced `${…}` template syntax (unbalanced `${` is rejected at construction)
- UTF-8 **byte** ceilings on email `subject_template` (1024) and `body_template` (65536); schema `maxLength` is a character ceiling for tooling and is not that byte bound
- Named `*_env` variables must be set and nonempty in the process environment at construction
...
| `max_concurrent_dispatches` | `8` | Bounded-concurrency semaphore for outbound notifications (`1..=4294967295`). When exhausted, alerts are dropped with a `warn!` rather than queued. |
...
- **Admission / failure policy.** The constructor always returns an error for invalid config (unknown keys, wrong types, empty URLs, out-of-range integers, malformed templates, incomplete SMTP credentials, and the other checks on this page and in [docs/notifications.md](notifications.md)). What happens next depends on the admission surface — `proxy_alerts` is registered `OptionalFailOpen`, and that posture is deliberate:
  - **Admin API create/update** still performs that construction and returns HTTP 400 for an invalid enabled config before storing it.
  - **File-mode load and `ferrum-edge validate`** warn (`Optional plugin config validation warning: …`) and admit the surrounding snapshot. `validate` can therefore exit 0 even when this plugin cannot be constructed. Inspect those warnings; a successful validation or startup does not prove the alert instance is active.
  - **Plugin-cache construction / reload** logs `Optional plugin omitted after validation failure: …` and omits this instance from the published cache rather than blocking publication.
  - **CP snapshot/delta admission** skips OptionalFailOpen construction failures instead of rejecting the generation (a serving data plane then omits the instance with a warning).
  - **Database serving-mode full load** quarantines an unconstructible OptionalFailOpen row: the process still starts, the instance is omitted from the cache, `config_rejected` is raised, and the stored row remains readable and deletable through the admin API. FailClosed / KeepLastKnownGood / unknown plugin names are never quarantined.
