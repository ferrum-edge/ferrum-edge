...
Bytes written **before** the relay starts are flushed before it starts. The relay owes a flush only for bytes it handed to a writer itself, so the two setup writes that can hand bytes to a `tokio-rustls` writer flush on their own: the TCP+TLS first-bytes forward (the decrypted opening bytes that inspection consumed, re-encrypted to a TLS backend leg) and WebSocket tunnel mode's forward of backend bytes that arrived with the `101` (written to a WSS client leg). Each flush runs inside the same bounds as its write — the setup deadline and trust fence for the prefix, the residual-forward timeout and session-stop arbiter for the WebSocket bytes — and a failed flush is reported exactly as the write it completes. Neither path writes or flushes anything when it has no bytes to forward. The outbound PROXY v2 header already flushed, and goes to a plain TCP socket.
...
- **Authorization lifetime (authenticated sessions only)**: A UDP/DTLS listener runs the full `on_stream_connect` admission chain, so a session can admit a gateway Consumer or an external identity together with that credential's own expiry. `on_stream_connect` runs **once**, at admission, and is never repeated, so an admitted session is bounded by the earliest of that credential's authoritative deadline and the finite `FERRUM_AUTHENTICATED_STREAM_MAX_LIFETIME_SECONDS` maximum. For plain UDP the maximum is anchored at **first-datagram session admission** — before the epoch resolve, the mesh egress decision, and the admission chain — so slow admission work consumes the session's own lifetime rather than extending it. Every awaitable post-admission setup stage runs under that bound (the first-datagram `on_udp_datagram` policy hooks, DNS resolution, the backend bind/connect, the backend DTLS handshake), and the plan is re-read before the session is committed: before any backend success is recorded, before the session map insert, before the reply task exists, and before the first backend send. Both directions are then enforced — the client→backend forward (the non-blocking inline admission, `last_client` fast path, the bounded hook-ingress worker, and the bounded per-session backend-send writer, which re-checks the plan at the commit after queueing) and the backend reply task. Relayed datagrams never refresh the deadline. Terminating frontend DTLS application replies complete only after the produced ciphertext datagrams are accepted by the UDP socket: the per-client driver owns every pending application `send_to` against the same absolute plan, drops that wait on expiry or cancellation (exact-deadline ties fail closed), and never flushes expired or cancelled queued application replies on shutdown. Enqueue into the DTLS driver is not a wire commit. At expiry the session's generation is marked expired before any cache or map reuse, only that exact generation is removed, sockets/tasks/channels are closed (cancelling an in-flight datagram hook), and the overload connection guard plus the listener's active-session slot are released exactly once. A setup-phase expiry releases a claimed HALF_OPEN circuit-breaker probe slot **neutrally**, so no backend is scored. The disconnect summary reports a client-side, backend-health-neutral decision carrying only the bounded `authorization.termination_reason` class (`credential_expired` or `authenticated_stream_max_lifetime`) — never an identity, credential, certificate field, expiry instant, or source address — and the same class increments the fixed-cardinality `authorization_lifetime` counters under the `stream_udp` family. **Unauthenticated UDP sessions are unaffected**: no principal was admitted, so no plan exists, no clock is read on the datagram path, and behavior is unchanged. See [response_body_streaming.md](response_body_streaming.md#authorization-lifetime-of-an-admitted-stream).
...
- **Backend send isolation / backpressure**: The shared listener receive/drain loop never awaits a backend send for an established session (issue #5045). An uncongested plain-UDP datagram keeps the borrowed, allocation-free fast path — one `try_send` straight out of the receive buffer, with no `Bytes` copy, no channel, and no task handoff. When the connected backend socket answers `WouldBlock` (a full LOCAL socket buffer, which is what tokio documents for `try_send`; a slow UDP application does not itself create transport backpressure) the datagram is handed to a bounded per-session backend-send writer, and every later datagram for that session queues behind it until the writer drains, so a datagram that arrived later can never overtake one already waiting. **DTLS origination always goes through that writer**: `DtlsConnection::send` completes only after the driver task has encrypted the plaintext and the socket has accepted every resulting ciphertext datagram, so leaving it inline made one session's driver scheduling a listener-wide dependency. An enqueue is deliberately **not** a wire commit — `datagrams_out` / `bytes_out` / per-session `bytes_sent` and the idle-activity stamp move only after the send settles, and the amplification response budget is accrued exactly once per client datagram (before the attempt that could emit it, never twice when a fast-path attempt hit `WouldBlock` and was requeued). The writer holds at most 64 datagrams and 256 KiB per session, with an additional 16 MiB retained-payload cap across the listener; both budgets stay charged until a dequeued datagram's send settles. Over-budget datagrams and datagrams for a session already being retired are tail-dropped, increment the listener's `egress_queue_drops` counter, and emit a rate-limited warning (first drop, then every 100th) with no per-client label cardinality. `egress_handoffs` and `egress_send_errors` separate real local socket pressure from queue drops and from backend send failures. Because a queued datagram commits later than the listener first saw it, destination ownership and NodeWaypoint source attribution are revalidated at the ACTUAL commit, and the authorization-lifetime gate runs both at admission and at the commit. Session stop/expiry closes the writer through the same `close_hook_ingress` call every teardown site already makes, sends nothing that is still queued, and drains residual item/byte accounting exactly once; a late datagram cannot respawn a writer for a retired session. Sessions with `on_udp_datagram` hooks are unchanged — they were already isolated by their per-session ingress worker (previous bullet), which still awaits its own forward.
...
- UDP `egress_queue_drops` / `egress_handoffs` / `egress_send_errors` (listener counters): client→backend datagrams dropped by a session's bounded backend-send writer, sends handed to that writer instead of written inline, and backend send failures the writer observed (see [UDP Session Management](#udp-session-management))
