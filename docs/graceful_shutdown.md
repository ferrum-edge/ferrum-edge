...
5. **Drain phase begins** — once every listener task has returned, the per-instance `draining` flag is set and the gateway logs `Shutdown drain begun: Connection: close hint and new-request rejection are active` at `info`. A proxy port that is already closed does **not** mean this step has run. The flag is set after the listener join on purpose: setting it earlier would reject new H3 streams and end accepted CONNECT-UDP tunnels during the HTTP/3 listener's drain window (see [HTTP/3](http3.md)). Setting the flag causes:
