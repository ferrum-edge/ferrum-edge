...
`mode=cp` on this chart is a **database-backed config distributor**, not a
Kubernetes CRD controller. The binary defaults `FERRUM_K8S_CONTROLLER_ENABLED`
(and pod discovery) to true inside a pod, but `ferrum-gateway` renders no
ClusterRole — those core watches would 403-retry for the life of the process.
The chart therefore emits `FERRUM_K8S_CONTROLLER_ENABLED=false` and
`FERRUM_K8S_POD_DISCOVERY_ENABLED=false` for `mode=cp` (`k8sController.enabled`
defaults to `false`; setting it `true` fails render). Watch Gateway API / Istio
CRDs with [`charts/ferrum-mesh`](../charts/ferrum-mesh) (`controlPlane.rbac`).

Multi-tenant control planes should set **`cp.requireNamespaceClaim: true`**
and **`admin.requireNamespaceClaim: true`** together on `charts/ferrum-gateway`
so CP↔DP gRPC JWTs and namespace-scoped admin routes both require an `ns` claim.
Enabling only the CP value does not constrain admin JWTs. See
[docs/cp_namespace_tenancy.md](cp_namespace_tenancy.md) and the chart README
...
**Injected sidecar lifecycle (issue [#4430](https://github.com/ferrum-edge/ferrum-edge/issues/4430)).** The webhook
...
feature gate). There is no ordinary-container fallback. Application probe ports
remain subject to inbound mesh capture; excluding a destination port would also
let ordinary application traffic on that port bypass mesh enforcement. Prefer
an `exec` probe that checks the application over loopback when a workload probe
must avoid capture. See [docs/mesh.md](mesh.md#sidecar-container).
