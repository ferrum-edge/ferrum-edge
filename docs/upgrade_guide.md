...
**Two boolean settings change meaning without refusing to start.** Everything else in this entry fails closed and names itself, but `FERRUM_POOL_ENABLE_HTTP2=0` and `FERRUM_POOL_ENABLE_HTTP_KEEP_ALIVE=0` previously failed to parse as booleans and fell back to `true`. They are now honoured as `false`. A gateway configured with either will start cleanly and silently disable backend HTTP/2 multiplexing, or backend keep-alive — which on the reqwest pool drops the TCP keepalive socket option and on the Sidecar-ingress Unix transport disables HTTP/1.1 connection reuse entirely, so every request dials a freshly admitted connection. Expect a latency and throughput regression that looks unrelated to the upgrade. Grep your environment and `ferrum.conf` for both before cutover.

**Operator action:** run `ferrum-edge validate` before upgrade and correct typos (`flase` → `false`) and out-of-range values. Accepted booleans are `true`/`false`/`1`/`0`. Idle per host must be 4–1024; H2 connections per host and max concurrent streams must be ≥ 1; stream/connection windows 65535–134217728; max frame 16384–1048576. If you set either boolean to `0` intending "leave it on", change it to `true`.
...
`X-Gateway-Error: backend_timeout` when a backend accepts a request and never reads the body. After #4074 that stopped happening once the local TCP send buffer absorbed the whole body: the upload pump saw a clean end-of-stream, dropped the write-timeout signal, and the request ran on to `backend_read_timeout_ms` or the client's own deadline. The watermark now survives that clean end-of-stream and stays live through the response-header wait when the most recent bridge reserve at end-of-stream was not blocked.

That response holdover is deliberately narrow. It arms only when the most recent bridge-capacity `reserve()` at end-of-stream obtained a permit without yielding. A `reserve()` poll that returns `Pending` is receive-window backpressure for that reserve; a later immediate permit clears it. The peer's receive window throttling the upload is positive evidence of consumption. The absence of backpressure at EOS plus a stalled response-header wait is the kernel-absorb signature.
