...
#### Admission parity with the data plane

Before a full snapshot or a delta can be accepted and broadcast, the CP runs the **same plugin construction gate** the data plane runs while staging its `PluginCache`. Plugin construction — not the JSON schema — is the real gate for most plugins (closed `config` objects, regex compilation, range checks), and the CP has no runtime `PluginCache` of its own, so without this a stored `plugin_configs` row that no serving mode can construct would be admitted and pushed to every DP. Each DP would then refuse the whole cache generation, keep serving last-known-good forever, and a DP that restarted or was newly scheduled would never obtain a base at all — while the CP still reported the snapshot published and the fleet connected, because `ConfigSync.Subscribe` is server-streaming with no client acknowledgement.

The gate mirrors `plugin_cache::try_create_plugin` exactly:
...
- **Snapshot apply rejection**: An admitted FULL_SNAPSHOT that fails the runtime apply (most commonly a plugin config the CP broadcast that this data plane cannot construct) logs the underlying validation errors — which name the plugin, its config id, and the constructor error — raises sticky `config_diverged` on `GET /cluster`, keeps last-known-good config serving, and terminates the subscription. This is deliberately distinct from *fencing*, which refuses a snapshot before apply and does not raise divergence. Because `ConfigSync.Subscribe` carries no client acknowledgement, this DP-side signal is the only in-band evidence that a data plane is frozen; the CP's `last_sync_at` records only that the CP sent the update.
