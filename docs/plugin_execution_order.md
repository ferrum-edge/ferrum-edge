...
4. **`ai_federation` (2985)** runs after the guard — it translates the OpenAI-format request to the matched provider's native format, calls the provider, normalizes the response back to OpenAI format, and returns via `RejectBinary`. Successful synthetic federation responses are passed through response-side body hooks before the client receives them, so `ai_semantic_firewall`, `ai_response_guard`, response transforms, and final-response hooks still apply. The plugin writes token metadata (`ai_total_tokens`, `ai_prompt_tokens`, `ai_completion_tokens`, `ai_model`, `ai_provider`) directly into `ctx.metadata` using the same keys as `ai_token_metrics`.
5. **`ai_token_metrics` (4100)** runs after the response comes back from the backend — it parses the LLM response body to extract token usage (prompt, completion, total, model) and writes it to `ctx.metadata`. This metadata flows into `TransactionSummary` for all downstream logging plugins. When `ai_federation` is active, `ai_federation` writes the same metadata keys directly and `ai_rate_limiter` records usage from that metadata on the rejection path.
