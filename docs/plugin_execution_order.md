...
3. **Bounded construction.** A producer materialises its replacement through a sink bounded by the same per-response retained ceiling, so an output that would exceed it is refused *while it is being written* rather than allocated in full and rejected afterwards. That construction refusal is **not** an ordinary no-op: `transform_response_body` returns `ResponseBodyTransformOutcome::{Unchanged, Replaced, CapacityRefused}` so the shared loop can install the neutral capacity terminal when a required rewrite/framing cannot fit, while non-owner / no-match paths stay `Unchanged`.
