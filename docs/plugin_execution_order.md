...
Gateway-generated gRPC-Web error terminals get those decorators too (#5747), so a browser client receives the CORS headers it needs to read the gRPC status instead of an opaque CORS failure. On HTTP/1.1 and HTTP/2 this covers every terminal of the native gRPC path's backend-error arm: backend unavailable, the gateway's own deadline (including an attempt budget that expires while the backend connection is acquired), an oversized or unretainable response, and a charged `backendRequest` budget expiry. On the HTTP/3 bridge to HTTP/1.1 and HTTP/2 backends it covers a classified backend dispatch failure, the route's own deadline, and an oversized response. The terminal is the gateway's decision, so its hooks run in the bounded charged-terminal mode: a response-replacing plugin is skipped, every other hook gets one poll, a hook's rejection is ignored, and a hook still pending after its poll finishes detached under the cleanup bound, ended early at the credential's lifetime, while every later hook still gets its poll. Each terminal keeps its wording, and an elapsed credential still gets the fixed authorization terminal. An HTTP/3 bridge response that proves too large while its body is collected, after `after_proxy` already decorated its head, carries that head's gateway decorations instead of running `after_proxy` again.
