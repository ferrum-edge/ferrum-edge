...
| `ferrum_mesh_hbone_tunnel_revocations_total` | counter | `proxy_id`, `reason`, `namespace` | `mesh` | `documented_only` | `conditional` | Live HBONE tunnels revoked by the admission fence because a later policy generation would no longer admit their CONNECT. |
