...
| `ferrum_mesh_tcp_received_bytes_total` | counter | Mesh identity, protocol, response flags, security policy, namespace | Request bytes received client->backend on closed connections (Istio `TCP_RECEIVED_BYTES` semantics). |
| `ferrum_mesh_tcp_sent_bytes_total` | counter | Mesh identity, protocol, response flags, security policy, namespace | Response bytes sent backend->client on closed connections (Istio `TCP_SENT_BYTES` semantics). |
...
| `ferrum_admin_audit_unknown_outcome_total` | counter | `namespace` | Intents whose trustworthy outcome observer was lost because a prior process exited or a request/settlement task was cancelled before an outcome was observed. The mutation may or may not have taken effect; the event is stored with `outcome = unknown_outcome` and must be reconciled by hand. |
...
| `ferrum_admin_audit_events_dropped_total` | counter | `reason`, `namespace` | Any non-zero value is an audit gap. `durable_handoff_failed` = the spool write failed; `no_durable_spool` = the deployment runs memory-only; `retained_capacity` = retained storage is full. Foreign-destination records are still durable in quarantine and are reported only by `ferrum_admin_audit_destination_mismatch_total`, not as dropped. |
...
| `ferrum_k8s_controller_config_publications_total` | counter | — | Reconciles that actually committed a changed configuration snapshot. `reconciliations_total` counts passes started, so only this one proves an observed cluster change became newly served config. Frozen here while `reconciliations_total` climbs means every pass found nothing to swap. |
| `ferrum_k8s_controller_watch_deletes_total` | counter | — | Watch `Delete` events observed across every scope. A withdrawn object that never bumps this, and never shows up as a relist, was never observed — that is a stalled watch, not a slow reconcile. A relist that drops the object delivers no `Delete`, so read this next to `watch_idle_relists_total`. |
...
| `ferrum_k8s_controller_watch_relist_missed_deletes_total` | counter | — | Objects an idle relist found missing from the store it replaced: withdrawals no `Delete` event delivered. The direct proof of a missed delete; the accompanying `warn` names the objects. The window it covers runs from the moment the retiring generation's stream was dropped to the successful list that replaced it, including every replacement attempt that timed out in between, so a small count on a churning scope can be an object deleted during that window rather than a stalled watch. |
| `ferrum_k8s_controller_watch_relist_missed_adds_total` | counter | — | Objects an idle relist found that the store it replaced never held: creations no `Apply` event delivered. Same warning, same caveat. |
| `ferrum_k8s_controller_watch_errors_total` | counter | — | Watch stream errors, one per attempt. A scope refused with HTTP 403 is held two minutes between attempts and any other failed initial list for a doubling interval up to 30 s (kube-rs's own backoff cannot space a failed list), so a steady climb is a scope that cannot succeed — usually an RBAC gap on a served kind. The scope is named by the `watch error` log line — at error level when the failure streak begins or changes class, at debug while it repeats — not by a label. |
...
| `ferrum_k8s_controller_status_request_timeouts_total` | counter | — | Status reads/writes abandoned on their own budget. Each one is an object left for the next reconcile, not a lost write. A sustained rise means the API server's status path is degrading before it costs whole reconcile rounds. |
| `ferrum_k8s_controller_status_batch_timeouts_total` | counter | — | Status patch batches that blew past the reconciler's defensive backstop. Batches bound themselves, so this should stay at zero; any increase means a patch path escaped its budget. |

**Deleted route still served:** scrape `watch_deletes_total` and `config_publications_total` around the deletion. Neither advancing means the control plane never observed the withdrawal (a stalled watch; recovery is bounded by `FERRUM_K8S_WATCH_IDLE_RELIST_SECS`). Both advancing while traffic continues means the control plane published and the fault is downstream — CP→DP distribution or listener rebuild — not the watch. If `watch_relist_missed_deletes_total` then advances on the next relist, the missed delete is proven and the accompanying warning names the object.
...
| `chargeback_sink_spool_bytes` | gauge | — | `api_chargeback_sink` | `documented_only` | `when_plugin_enabled` | Chargeback sink on-disk owned spool bytes (active, temp, in-flight claim, corrupt, and dead-lettered files). |
| `chargeback_sink_spool_dead_letter_rows_total` | counter | — | `api_chargeback_sink` | `documented_only` | `when_plugin_enabled` | Permanently rejected rows durably handed to the private dead-letter payload store. |
...
| `chargeback_sink_spool_files` | gauge | — | `api_chargeback_sink` | `documented_only` | `when_plugin_enabled` | Chargeback sink on-disk owned spool file count (active, temp, in-flight claim, corrupt, and dead-lettered files). |
...
| `chargeback_sink_spool_replay_attempts_total` | counter | — | `api_chargeback_sink` | `documented_only` | `when_plugin_enabled` | ClickHouse HTTP attempts made by bounded spool replay, including permanent-failure isolation children. |
...
| `ferrum_admin_audit_destination_mismatch_total` | counter | `namespace` | `admin_audit` | `documented_only` | `always` | Durable admin audit records quarantined because they target a different database or namespace destination. |
...
| `ferrum_cp_grpc_active_node_ids` | gauge | `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Distinct admitted CP configuration-stream node state keys. |
| `ferrum_cp_grpc_active_streams` | gauge | `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Currently active long-lived CP configuration streams across ConfigSync, MeshSubscribe, and ADS. |
...
| `ferrum_cp_grpc_stream_admission_rejections_total` | counter | `surface`, `reason`, `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Long-lived CP configuration streams rejected at admission, by bounded surface and reason. |
...
| `ferrum_dp_config_stale` | gauge | `namespace` | `dp_config` | `alert` | `conditional` | Whether the DP's applied configuration is past its bound with no authoritative CP (1) or not (0). |
...
| `ferrum_dp_cp_admission_refused_total` | counter | `namespace` | `dp_config` | `documented_only` | `conditional` | ConfigSync subscriptions the control plane refused with RESOURCE_EXHAUSTED because a CP gRPC stream admission budget is saturated. The CP is alive, so a refusal is recorded as reconnecting and never latches the stale-config bound. |
...
| `ferrum_frontend_client_trust_fenced_total` | counter | `scope`, `namespace` | `tls` | `documented_only` | `conditional` | Requests or streams refused before routing because the transport's client-certificate trust was withdrawn. |
| `ferrum_frontend_client_trust_generation` | gauge | `scope`, `namespace` | `tls` | `documented_only` | `conditional` | Monotonic frontend client-CA/CRL trust generation currently in force for a listener scope. |
| `ferrum_frontend_client_trust_publications_total` | counter | `scope`, `outcome`, `namespace` | `tls` | `documented_only` | `conditional` | Accepted frontend client-trust reload publications by bounded outcome. |
| `ferrum_frontend_client_trust_rejected_candidates_total` | counter | `scope`, `namespace` | `tls` | `documented_only` | `conditional` | Frontend client-trust reload candidates refused; the last accepted generation and its sessions are retained. |
...
| `ferrum_frontend_tls_handshake_failures_total` | counter | `reason`, `namespace` | `tls` | `alert` | `always` | Frontend TLS handshakes refused before any HTTP work, by bounded reason. |
...
| `ferrum_k8s_controller_config_publications_total` | counter | — | `k8s_controller` | `documented_only` | `conditional` | Kubernetes reconciles that committed a changed gateway configuration snapshot. |
...
| `ferrum_k8s_controller_status_batch_timeouts_total` | counter | — | `k8s_controller` | `documented_only` | `conditional` | Kubernetes status patch batches abandoned by the reconciler's defensive outer timeout after the batch failed to bound itself. |
| `ferrum_k8s_controller_status_request_timeouts_total` | counter | — | `k8s_controller` | `documented_only` | `conditional` | Kubernetes status reads and writes abandoned because they exceeded their wall-clock budget on the serialized reconcile loop. |
| `ferrum_k8s_controller_watch_deletes_total` | counter | — | `k8s_controller` | `documented_only` | `conditional` | Kubernetes watch Delete events observed across every watched scope. |
| `ferrum_k8s_controller_watch_errors_total` | counter | — | `k8s_controller` | `documented_only` | `conditional` | Kubernetes watch stream errors observed across every watched scope, including refused initial lists and failed watch starts. |
...
| `ferrum_k8s_controller_watch_relist_missed_adds_total` | counter | — | `k8s_controller` | `documented_only` | `conditional` | Objects an authoritative Kubernetes relist found that the watch scope's previous store never held: creations no Apply event delivered. |
| `ferrum_k8s_controller_watch_relist_missed_deletes_total` | counter | — | `k8s_controller` | `documented_only` | `conditional` | Objects an authoritative Kubernetes relist found missing from the watch scope's previous store: withdrawals no Delete event delivered. |
...
| `ferrum_mesh_app_probe_active_connections` | gauge | `gateway_namespace` | `mesh_app_probe` | `documented_only` | `conditional` | Application-probe listener connections currently admitted. |
| `ferrum_mesh_app_probe_active_probes` | gauge | `gateway_namespace` | `mesh_app_probe` | `documented_only` | `conditional` | Loopback application probes executing right now. |
| `ferrum_mesh_app_probe_max_active_probes` | gauge | `gateway_namespace` | `mesh_app_probe` | `documented_only` | `conditional` | Configured concurrent loopback application-probe budget (0 = unlimited). |
| `ferrum_mesh_app_probe_max_connections` | gauge | `gateway_namespace` | `mesh_app_probe` | `documented_only` | `conditional` | Configured application-probe listener connection cap (0 = unlimited). |
| `ferrum_mesh_app_probe_max_connections_per_ip` | gauge | `gateway_namespace` | `mesh_app_probe` | `documented_only` | `conditional` | Configured per-source-IP application-probe connection cap (0 = unlimited). |
| `ferrum_mesh_app_probe_rejected_connections_total` | counter | `reason`, `gateway_namespace` | `mesh_app_probe` | `documented_only` | `conditional` | Application-probe connections refused in the accept loop before any task or HTTP state was allocated, by reason. |
| `ferrum_mesh_app_probe_rejected_probes_total` | counter | `gateway_namespace` | `mesh_app_probe` | `documented_only` | `conditional` | Probe requests refused because the active-probe budget was full. |
| `ferrum_mesh_app_probe_total` | counter | `container`, `probe`, `outcome`, `gateway_namespace` | `mesh_app_probe` | `documented_only` | `conditional` | Rewritten kubelet application probes performed by the sidecar against the application over loopback, by bounded outcome. |
| `ferrum_mesh_bpf_accept_to_first_byte_microseconds` | histogram | `le` | `mesh_bpf` | `documented_only` | `when_plugin_enabled` | Time from captured TCP passive-established accept to the first inbound application-data byte on the same accepted socket, in microseconds. Missing, raced, stale, reversed, or corrupt correlation evidence emits no sample; fixed le buckets plus sum/count; sum overflow or counter saturation drops the sample. |
...
| `ferrum_mesh_bpf_drops_total` | counter | `reason` | `mesh_bpf` | `documented_only` | `when_plugin_enabled` | Connection-bypass decisions by reason, produced by the connect4/connect6 capture hooks and counted on per-CPU kernel counters, so an overrun of the event ringbuf cannot silently lose a decision. |
...
| `ferrum_mesh_bpf_srtt_microseconds` | histogram | `le` | `mesh_bpf` | `documented_only` | `when_plugin_enabled` | TCP smoothed RTT samples in microseconds. Fixed le buckets plus sum/count; zero samples are ignored and sum overflow or counter saturation drops the sample. |
| `ferrum_mesh_bpf_syn_to_ack_microseconds` | histogram | `le` | `mesh_bpf` | `documented_only` | `when_plugin_enabled` | Time between SYN send and ACK observation in microseconds. Fixed le buckets plus sum/count; zero samples are ignored and sum overflow or counter saturation drops the sample. |
...
| `ferrum_mesh_hbone_inner_pool_events_total` | counter | `protocol`, `event`, `namespace` | `mesh` | `documented_only` | `conditional` | Inner application connections reused inside fenced HBONE tunnels, by inner wire protocol and outcome (hit, miss, eviction, discard, fenced). |
...
| `ferrum_mesh_hbone_tunnel_revocations_total` | counter | `proxy_id`, `reason`, `namespace` | `mesh` | `documented_only` | `conditional` | Live HBONE tunnels revoked by the admission fence because the current policy generation, peer credential, or enforced revocation list would no longer admit their CONNECT, or would no longer let it carry later application operations. |
...
| `ferrum_mesh_metric_series_overflow_total` | counter | `family`, `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Mesh metric series admissions dropped because the per-family live-series budget (prometheus_metrics.mesh_series_budget_per_family) was exhausted. The budget is shared by ordinary identity series and CEL-derived keys; overflow has fixed family labels only. |
...
| `ferrum_mesh_node_waypoint_capture_active_connections` | gauge | `gateway_namespace` | `mesh_node_waypoint` | `documented_only` | `conditional` | Captured inbound connections currently holding an admission permit. |
| `ferrum_mesh_node_waypoint_capture_max_connections` | gauge | `gateway_namespace` | `mesh_node_waypoint` | `documented_only` | `conditional` | Configured capture-listener connection cap (0 = unlimited). |
| `ferrum_mesh_node_waypoint_capture_max_connections_per_ip` | gauge | `gateway_namespace` | `mesh_node_waypoint` | `documented_only` | `conditional` | Configured per-source-IP capture-listener connection cap (0 = unlimited). |
| `ferrum_mesh_node_waypoint_capture_rejected_connections_total` | counter | `reason`, `gateway_namespace` | `mesh_node_waypoint` | `documented_only` | `conditional` | Captured inbound connections refused in the accept loop before a task was allocated, by bounded reason. |
...
| `ferrum_mesh_request_bytes` | histogram | `source_workload`, `source_namespace`, `source_principal`, `source_app`, `source_service`, `destination_workload`, `destination_namespace`, `destination_principal`, `destination_app`, `destination_service`, `request_protocol`, `response_code`, `response_flags`, `connection_security_policy`, `gateway_namespace`, `le` | `mesh` | `documented_only` | `conditional` | Mesh HTTP/gRPC request body size in bytes. |
...
| `ferrum_mesh_request_messages_total` | counter | `source_workload`, `source_namespace`, `source_principal`, `source_app`, `source_service`, `destination_workload`, `destination_namespace`, `destination_principal`, `destination_app`, `destination_service`, `request_protocol`, `response_code`, `response_flags`, `connection_security_policy`, `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Mesh gRPC request messages. |
...
| `ferrum_mesh_response_bytes` | histogram | `source_workload`, `source_namespace`, `source_principal`, `source_app`, `source_service`, `destination_workload`, `destination_namespace`, `destination_principal`, `destination_app`, `destination_service`, `request_protocol`, `response_code`, `response_flags`, `connection_security_policy`, `gateway_namespace`, `le` | `mesh` | `documented_only` | `conditional` | Mesh HTTP/gRPC response body size in bytes. |
| `ferrum_mesh_response_messages_total` | counter | `source_workload`, `source_namespace`, `source_principal`, `source_app`, `source_service`, `destination_workload`, `destination_namespace`, `destination_principal`, `destination_app`, `destination_service`, `request_protocol`, `response_code`, `response_flags`, `connection_security_policy`, `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Mesh gRPC response messages. |
| `ferrum_mesh_slice_drift_data_planes` | gauge | `state`, `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Tracked mesh data planes by CP-side slice convergence state. |
| `ferrum_mesh_slice_drift_tracked_data_planes` | gauge | `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Total mesh data planes retained in the CP slice-drift registry. |
...
| `ferrum_mesh_udp_placement_migration_adoption_proof` | gauge | `proof`, `gateway_namespace` | `mesh_udp_migration` | `documented_only` | `conditional` | Node-specific proof that authorized a recordless Ambient UDP placement adoption (one bounded proof is 1). |
| `ferrum_mesh_udp_placement_migration_adoptions_total` | counter | `proof`, `gateway_namespace` | `mesh_udp_migration` | `documented_only` | `conditional` | Recordless Ambient UDP placement adoptions by bounded node-specific proof. |
| `ferrum_mesh_udp_placement_migration_established_adoptions_total` | counter | `gateway_namespace` | `mesh_udp_migration` | `documented_only` | `conditional` | Ambient UDP placements adopted without a same-incarnation node-local durable record, summed over every node-specific proof; equals the sum of ferrum_mesh_udp_placement_migration_adoptions_total. |
...
| `ferrum_mesh_workload_api_active_connections` | gauge | `gateway_namespace` | `mesh` | `documented_only` | `conditional` | SPIFFE Workload API connections currently admitted by the transport admission boundary. |
| `ferrum_mesh_workload_api_active_rpcs` | gauge | `gateway_namespace` | `mesh` | `documented_only` | `conditional` | SPIFFE Workload API RPCs currently admitted by RPC admission (service-wide ceiling and per-peer-UID quota); counts service-dispatched RPC streams only — each occupies one HTTP/2 stream, but streams refused inside HTTP/2 or rejected before service dispatch are not observable here. |
| `ferrum_mesh_workload_api_connections_closed_total` | counter | `reason`, `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Established SPIFFE Workload API connections closed by a transport deadline or the bounded shutdown force-close, by fixed reason. |
| `ferrum_mesh_workload_api_connections_rejected_total` | counter | `reason`, `gateway_namespace` | `mesh` | `documented_only` | `conditional` | SPIFFE Workload API connections refused before any tonic allocation, by fixed admission reason. |
| `ferrum_mesh_workload_api_rpcs_rejected_total` | counter | `gateway_namespace` | `mesh` | `documented_only` | `conditional` | SPIFFE Workload API RPCs shed by RPC admission — the service-wide concurrency ceiling or the per-peer-UID quota — before any producer work was spawned; the HTTP/2 stream is already open and carries a RESOURCE_EXHAUSTED result, so this is not a protocol-level stream refusal. |
...
| `ferrum_node_agent_cni_calls_total` | counter | `verb`, `outcome`, `namespace` | `node_agent` | `documented_only` | `conditional` | Node-agent CNI plugin RPC outcomes by verb. |
...
| `ferrum_node_agent_ingress_interface_configured_interfaces` | gauge | `namespace` | `node_agent` | `documented_only` | `conditional` | Number of explicitly configured ingress interfaces (names are never labels). |
| `ferrum_node_agent_ingress_interface_expected_interfaces` | gauge | `namespace` | `node_agent` | `documented_only` | `conditional` | Number of interfaces required by the proved node/CNI route topology (names are never labels). |
| `ferrum_node_agent_ingress_interface_family_covered` | gauge | `family`, `namespace` | `node_agent` | `documented_only` | `conditional` | Whether the required route family is currently proved complete. |
| `ferrum_node_agent_ingress_interface_family_required` | gauge | `family`, `namespace` | `node_agent` | `documented_only` | `conditional` | Whether a route family must be covered by the configured interface set. |
| `ferrum_node_agent_ingress_interface_topology` | gauge | `state`, `reason`, `namespace` | `node_agent` | `documented_only` | `conditional` | NodeWaypoint ingress-interface topology validation state and bounded reason. |
...
| `ferrum_oauth2_introspection_cache_admission_skips_total` | counter | `class`, `reason`, `namespace` | `oauth2_introspection` | `documented_only` | `always` | OAuth2 introspection results not retained because a bounded cache admission could not complete. |
| `ferrum_oauth2_introspection_cache_cleanup_work_total` | counter | `namespace` | `oauth2_introspection` | `documented_only` | `always` | Bounded OAuth2 introspection eviction-index tickets examined. |
| `ferrum_oauth2_introspection_cache_entries` | gauge | `class`, `namespace` | `oauth2_introspection` | `documented_only` | `always` | OAuth2 introspection cache entries retained across live plugin generations. |
| `ferrum_oauth2_introspection_cache_evictions_total` | counter | `class`, `reason`, `namespace` | `oauth2_introspection` | `documented_only` | `always` | OAuth2 introspection cache entries evicted by bounded cleanup or capacity admission. |
| `ferrum_oauth2_introspection_cache_retained_bytes` | gauge | `class`, `namespace` | `oauth2_introspection` | `documented_only` | `always` | OAuth2 introspection cache bytes retained across live plugin generations, including fixed eviction indexes. |
...
| `ferrum_rate_limit_enforcement_unavailable_total` | counter | `namespace` | `prometheus_metrics` | `documented_only` | `always` | Requests refused because centralized rate-limit enforcement was unavailable. |
| `ferrum_rate_limit_exceeded_total` | counter | `namespace` | `prometheus_metrics` | `dashboard` | `always` | Total rate limit rejections. |
| `ferrum_rate_limit_local_fallback_decisions_total` | counter | `namespace` | `prometheus_metrics` | `documented_only` | `always` | Rate-limit decisions taken on the per-process fallback budget. |
...
| `ferrum_tls_revocation_expiry_seconds` | gauge | `material_id`, `kind`, `source_kind`, `namespace` | `tls` | `alert` | `conditional` | Seconds until the revocation material nextUpdate. Negative means expired; expired material is refused at reload and at startup. |
