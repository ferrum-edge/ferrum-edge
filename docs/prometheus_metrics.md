...
| `ferrum_mesh_slice_convergence_diverged` | gauge | `gateway_namespace` | `mesh` | `documented_only` | `always` | Number of tracked mesh identities whose last sent slice version is not acknowledged. |
| `ferrum_mesh_slice_convergence_identities` | gauge | `gateway_namespace` | `mesh` | `documented_only` | `always` | Number of mesh identities retained in the CP slice-convergence tracker (connected plus retention window). |
| `ferrum_mesh_slice_status_reports_total` | counter | `outcome`, `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Control-plane MeshSlice status reports by closed-set outcome (applied/rejected/stale_version/unknown_version/unknown_identity/identity_mismatch). |
