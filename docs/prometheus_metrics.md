...
| `ferrum_k8s_controller_watch_errors_total` | counter | — | Watch stream errors, one per attempt. kube-rs backs off between attempts and a scope refused with HTTP 403 is additionally held two minutes, so a steady climb is a scope that cannot succeed — usually an RBAC gap on a served kind. The scope is named by the once-per-transition `watch error` log line, not by a label. |
