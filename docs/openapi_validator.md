...
`openapi_validator` turns an attached OpenAPI or Swagger document into a runtime JSON contract for HTTP proxies. It is normally generated by the API spec importer when a spec opts in with `x-ferrum-validate`; operators should not hand-author the large `operations` block except for tests or file-mode experiments.

The plugin runs at priority `2960`, after `body_validator` and before AI request policy plugins. It compiles path regexes and JSON Schemas when the plugin config loads, then validates only matching buffered request and response bodies.
...
| `request_content_types` | `["application/json"]` | Request media types in scope. Parameters such as `; charset=utf-8` are stripped before matching. |
| `response_content_types` | `["application/json"]` | Response media types in scope. |
