...
The plugin has priority `2960`. Its dedicated client-contract phase runs before `body_validator` request checks in `before_proxy`; priorities order hooks within each phase. Unknown-operation admission runs in `before_proxy`, backend-request fallback in `on_final_request_body`, and response validation in `on_final_response_body`. It compiles path regexes and JSON Schemas when the plugin config loads, then parses and validates matching buffered request and response bodies.
...
Trailing slashes on the listen prefix are removed when joining non-root paths: `/p2/oas2/` plus `/items/{id}` produces `/p2/oas2/items/{id}` and `^/p2/oas2/items/[^/]+$`. The listen prefix is always literal in the regex. With no server base, the Paths-key root `/` preserves the literal listen path so it matches the router's route key: `/p2/oas2` yields `^/p2/oas2$`, while `/p2/oas2/` yields `^/p2/oas2/$`. Any other Paths-key trailing slash is preserved (`/items/` produces `/p2/oas2/items/`). Root (`/`) and host-only proxies add no prefix. Exact (`=...`) and regex (`~...`) listen routes also add no prefix; their spec's server/basePath and Paths keys must describe the full inbound paths.
...
| `fail_on_unknown_operation` | `true` | Reject requests that do not match any generated operation. In `block` mode the client-visible problem `title` is `Unknown OpenAPI operation` with detail `No OpenAPI operation matched this request`; the method and path are not interpolated into either string. |
...
- Media selectors and map keys (`request_body.content`, each `responses[status]` entry) use MIME tokens, including braces, and accept `type/subtype`, `type/*`, or `*/*`. The constructor rejects control characters, trims surrounding Unicode whitespace from the base type, lowercases ASCII, and ignores parameters after the first semicolon. List entries and map keys must be unique after this normalization; JSON Schema can check only literal duplicates.
...
`max_body_bytes` is an integer from 1 through 18446744073709551615 and `error_truncate_chars` from 0 through 18446744073709551615, also bounded by the target's `usize` range. Operation `method`, `path_template`, `path_regex`, and an explicit `operation_label` must be nonempty, as must entries in `bypass.paths`, `bypass.methods`, and `bypass.consumers`. Regex syntax is checked at construction. Error status codes are integers from 400 through 599. `error_response.content_type` and Header Object content keys use concrete RFC 9110 media syntax: no wildcard type/subtype or leading whitespace, and each parameter requires a token name and token or quoted-string value.
...
- `text/*` bodies validate as UTF-8 strings. UTF-8 binary bodies receive full string-schema validation. Non-UTF-8 binary bodies support boolean schemas and a byte-oriented subset: optional string-compatible `type`, `minLength`/`maxLength` in bytes, and `format: binary`. The permitted annotations are `$schema`, `$id`, `$anchor`, `$comment`, `$defs`, `definitions`, `title`, `description`, `default`, `example`, `examples`, `deprecated`, `readOnly`, `writeOnly`, and `xml`. Other keywords, including references, compositions, conditionals, `not`, `const`, `enum`, and `pattern`, require a text representation and cause non-UTF-8 bodies to fail validation. This rule applies equally to requests and responses; use a multipart metadata-object schema for arbitrary binary file content.
