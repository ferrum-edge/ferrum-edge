...
**Cleanup mechanism:** TTL-based expiration (`ttl_seconds` config field). Completed-response retention is bounded by `max_entry_size_bytes` and exact local `max_total_size_bytes` accounting; oversized or over-cap responses return normally, clear their local in-flight marker, and are not retained. When the local cache exceeds `max_entries`, expired entries are evicted first, then oldest completed entries are removed. Active in-flight markers are not evicted while live. When using `sync_mode: "redis"`, completed responses are stored in Redis with TTL-based key expiration only after the same per-entry size admission; oversized responses are not serialized or written. Responses skipped only by the local total cap still attempt Redis publication, and the distributed in-flight lock is released only when that publication succeeds or is skipped by per-entry admission.
