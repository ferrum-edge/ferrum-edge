...
| `BackendRequest` | `ConnectionReset` | hyper `send_request` failed post-handshake — including `is_canceled`, because cancellation does not prove that the backend did not accept the request. This is **post-wire** by definition. Excluded from `is_connect_class()` so `retry_on_connect_failure` cannot replay non-idempotent POSTs; canceled pooled senders are still invalidated before the error is returned |
