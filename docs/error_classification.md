...
## Connect / setup / cancel / reset boundary (HTTP reqwest)

**The phase comes from the caller, never from the error (issue #4536).** A pre-wire class is a licence to replay a request past `retryable_methods`, so it may only be issued on evidence the gateway itself controls: a code path that knows it is still dialing/handshaking (`phase_is_connect = true`), or a typed contract from a library that the request was never dispatched (hyper `is_canceled`). A rustls variant, an alert description, or an errno seen *after* reqwest's connect phase are all peer- or timing-controlled and are **not** phase evidence — rustls raises `AlertReceived(_)` for a fatal alert at any point and `InappropriateHandshakeMessage` for a post-handshake handshake message (TLS 1.3 post-handshake client auth, RFC 8446 §4.6.2), both of which an origin can send after reading a complete POST body.

`reqwest::Error::is_connect()` is TCP-only, so reqwest's shared HTTP/1 pool cannot report the TLS phase at all. When rustls is **not** in the request chain, hyper `is_canceled()` is still a pre-wire pool cancel:
...
| (2) Setup-phase TLS | Typed `rustls::Error` while the caller independently knows it is still connecting/setting up | `TlsError` | `false` |
...
| (4) Post-connect ambiguous failure | RST, refusal, `EADDRNOTAVAIL` or rustls error (including a handshake-named alert) when no independent setup-phase evidence exists and the request was not canceled | `ConnectionReset` | `true` |
...
Omitted TLS `close_notify` is teardown (#4051): `UnexpectedEof` whose Display contains `without sending TLS close_notify` is matched **before** rustls so it cannot become `TlsError`. HTTPS-to-plaintext remains `TlsError` (#4053). When the handshake error is the characteristic plaintext-HTTP shape (`rustls::InvalidMessage::InvalidContentType` / `UnknownProtocolVersion`, OpenSSL "wrong version number", an HTTP/1.x status line during TLS), operators get a rate-limited WARN naming the proxy, the backend target, and `error_reason=https_to_plaintext_backend` with the hint to set `backend_scheme: http`. The client-facing 502 body is unchanged.
...
|---|---|---|---|
| HTTP/1.1 (reqwest) | [`classify_reqwest_error`](../src/retry.rs) | `&reqwest::Error` | `is_connect()` / `is_timeout()` typed methods → typed source-chain walk for io/TLS/DNS. Port exhaustion and every other pre-wire io verdict live INSIDE the `is_connect()` branch. **Four-way boundary:** (1) connect-phase RST/refused with no rustls → `ConnectionRefused`; (2) typed rustls during the independently known connect phase → `TlsError`; (3) hyper `is_canceled()` with no rustls → `ConnectionPoolError` (issue #4406 live reqwest path; issue #3578); (4) post-connect RST / refusal / `EADDRNOTAVAIL` / rustls error → `ConnectionReset` (issue #4536). Missing `close_notify` is matched *before* rustls so it stays `ConnectionClosed` (#4051). Post-connect `UnexpectedEof` / incomplete message → `ConnectionClosed`. Bounded substring fallback inside the `is_connect()` branch |
