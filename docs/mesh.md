...
1. **Gateway SVID backs the inbound server identity.** When no explicit `FERRUM_FRONTEND_TLS_CERT_PATH` / `KEY_PATH` is set, the gateway SVID's leaf + key are loaded as the inbound listener's server certificate. A mesh configured with only `FERRUM_GATEWAY_SVID_*` therefore presents that SVID as its inbound server cert and serves mTLS, instead of falling open to plaintext under the default PERMISSIVE mode (previously the SVID backed only the *peer verifier*, not the server cert). **Rotation caveat:** this SVID-backed inbound server cert is loaded once at startup and **pinned for the process lifetime**. The gateway SVID file watcher (`run_gateway_svid_file_rotation_loop`) rotates only the *backend* (outbound) identity, so after a file-based SVID rotation the inbound listener keeps presenting the startup leaf until the gateway is restarted — and inbound mTLS will fail once that pinned leaf expires (~one rotation period). A startup `warn!` flags this. For unattended file-based rotation, supply explicit `FERRUM_FRONTEND_TLS_*` (with your own rotation) or restart / rolling-redeploy on rotation. Auto-rotating the SVID-backed *inbound* identity (a live `ResolvesServerCert`) is tracked with the deferred `FERRUM_MESH_CA_BACKEND` rotation work below.
...
Wiring `FERRUM_MESH_CA_BACKEND` (`spire` / `internal`) to issue and rotate a runtime SVID into the data plane remains the residual follow-up; today only file-based gateway SVID material provides a runtime workload identity.
