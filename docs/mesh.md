...
- Stream-family (TCP/UDP/DTLS) upstreams engage per-port `loadBalancer` / `localityLbSetting` / `maxEjectionPercent` at selection time when all upstream targets share a single port (i.e. the LB cache resolves a non-zero `initial_dispatch_port_override`), matching the pre-selection semantics the HTTP path uses. Per-port `outlierDetection` *thresholds* (consecutive errors, base ejection time, etc.) are not recorded for stream paths — raw relay sessions carry no response status — so ejection state is driven solely by HTTP traffic sharing the same upstream. A mixed HTTP + stream upstream therefore benefits from DR ejection recorded by the HTTP side; stream-only upstreams see no ejection without a companion HTTP proxy. Per-port `connectTimeout` and `maxConnections` apply to stream dial independently of this change (they are post-selection).
