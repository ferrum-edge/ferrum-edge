...
poll failures preserve the last-good bundle in both modes. The effective trust
set is used for outbound mTLS and for inbound SPIFFE peer verification; a
polled trust domain is no longer outbound-only. Endpoints are validated at
slice apply for SSRF (link-local / loopback / cloud metadata IPs are rejected)
and must use `https://`; response bodies are capped at 2 MiB and parsed
bundles are capped at 256 X.509 + 256 JWT authorities.
...
| `FERRUM_MESH_FEDERATION_FAIL_OPEN` | `false` | Federation bootstrap policy. `false` blocks CP-supplied fallback bundles for remotes with `federation_endpoint` until the poller installs a last-good bundle. `true` allows CP fallback during bootstrap. Both modes preserve last-good polled bundles across transient failures |
