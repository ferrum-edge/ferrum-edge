...
**One budget per establishment, opened before admission.** That deadline is created as an absolute instant at the very top of a checkout — before path admission, before any pool creation-lock wait, before `connect(2)`, before the protocol handshake, and (for a WebSocket) before the RFC 6455 upgrade exchange — and every one of those stages is bounded by that same instant. No stage derives a second budget from `backend_connect_timeout_ms`, so an establishment can never consume two or three full setup budgets. The synchronous admission `stat`s cannot be preempted while they execute, but the time they cost is charged all the same: the next bounded await resolves immediately once the deadline has passed. An operator duration the platform clock cannot represent still fails closed rather than becoming an unbounded wait.
...
**`connectionPool.tcp.maxConnections`.** Pooling is not a way around the DestinationRule ceiling. A slot on the gateway-wide per-destination counter (`src/backend_conn_limit.rs`) is taken at the one point a **new physical connection** is about to be constructed — never on a pool hit and never per multiplexed h2c stream, so a destination pinned at `maxConnections: 1` still serves an unbounded number of sequential HTTP/1.1 requests and concurrent RPCs over its one admitted connection. The lane is resolved from the LB-selected target's dial host and its exact DestinationRule **policy** port (a `targetPort` remap included), so a Unix carrier shares one ceiling with the same destination's WebSocket, raw-TCP, reqwest, and pooled-H2 connections instead of getting a second one. The guard is owned by the connection's own driver task, so the slot is held for exactly that socket's lifetime — idle residence included — and released on handshake failure, close, eviction, withdrawal, drain, and shutdown. An over-cap refusal is decided **before `connect(2)`**: no socket is opened and no application byte is written, and it surfaces as a typed `503` with `ErrorClass::BackendConnectionLimit` — pre-wire, health-neutral (a saturated destination is not an unhealthy one), and retryable onto another target with its own lane.

**Keep-alive.** `FERRUM_POOL_ENABLE_HTTP_KEEP_ALIVE` (and the per-proxy `pool_enable_http_keep_alive` override) is honored literally for HTTP/1.1 on this transport: with it off, a carrier is neither taken from nor returned to the idle set, so every request gets its own freshly admitted connection. h2c is unaffected — HTTP/2 has no keep-alive negotiation, and stream multiplexing is the transport's defining behavior rather than a keep-alive optimization.

**Metrics.** The pool exports physical connects, hits, misses, identity retirements, setup failures, pre-dial checkout failures (path admission and over-cap refusals), and gauges for idle HTTP/1.1 carriers, shared h2c carriers, and open physical connections. They appear on the JWT-authenticated runtime metrics endpoint under `connections.unix_backend_pool` (`GET /metrics/runtime`). Cardinality is fixed: one object for the whole pool, never a per-target label. Every field is one atomic read — the gauges are maintained at each insert/removal and at each connection-driver spawn/completion, so producing the snapshot never scans a map and the request path never pays for it.
