...
- **Egress is materialized for both**, per transport: **Ambient outbound** builds per-service HBONE routes (a host-routed `/` proxy per in-mesh Service → `mesh.hbone`-tagged upstreams dialing the destination's `:15008`); **Sidecar outbound** builds per-service SVID-mTLS routes (`mesh.mtls`-tagged upstreams dialing the destination sidecar's inbound `:15006` with plain HTTP/2 over mutual TLS — never HBONE). Either way the upstream targets carry the destination workload's SPIFFE id (`mesh.spiffe_id`), which the outbound handshake **pins**: the peer must present exactly that SVID, not merely one from an allowed trust domain. Sidecar egress yields to the local workload's own inbound loopback route (the route table holds one proxy per host+path), so a service's own-sidecar traffic stays local. Non-convention transport ports are configured with `FERRUM_MESH_EGRESS_HBONE_PORT` / `FERRUM_MESH_EGRESS_MTLS_PORT`.
  ...
  - **Raw-TCP egress (original-destination routing, Ambient).** Stream-family service ports (`tcp`, `tls`, and the DB protocols — anything not HTTP-family) materialize **per-port upstreams only**, no route proxies: a raw stream has no Host header to route by. Instead the accept loop on the outbound capture listener matches the captured original destination **strictly against `(service VIP, service port)`** — `MeshService.cluster_ips`, populated from `Service.spec.clusterIPs` by the Kubernetes translator (file-source operators set `cluster_ips` directly) — and relays the raw byte stream through an HBONE CONNECT tunnel to the LB-selected workload's app port (identity-pinned, capability-gated like every HBONE dial; the destination's transparent inbound relay needs no changes). Port-number-only matching is deliberately unsupported: two services may share a port number, and a captured dial to a non-mesh destination must never be tunnelled into the mesh on a coincidence. A captured destination matching a declared pair whose upstream did not materialize is **closed** (never guessed); anything else falls through to the HTTP path unchanged. VIP-less (headless) services and **Sidecar** topology raw-TCP egress are not routable (warned at materialization; Sidecar's mTLS-HTTP transport carries no raw byte stream and its peer inbound has no stream listener — a later stage). Per-port `DestinationRule` traffic policy applies to the TCP upstreams exactly as to the HTTP ones; passive-health/outlier recording is absent on the raw relay (no response status to classify), matching the stream-proxy paths. UDP is out of scope (`SO_ORIGINAL_DST` REDIRECT does not apply to the UDP capture model).
...
`FERRUM_MESH_CA_BACKEND=spire` delegates SVID issuance to a SPIRE Agent via the SPIFFE Workload API. At mesh startup the data plane connects to the SPIRE Agent socket, fetches an X.509 SVID for its workload identity, and **rotates it live** into the running data plane's `gateway_svid_bundle` slot — the same slot the gateway SVID *file* watcher feeds. So the inbound mTLS server certificate (when no explicit `FERRUM_FRONTEND_TLS_*` is set) and the backend (outbound) client identity both come from SPIRE and re-key on every rotation with no restart: the inbound `ResolvesServerCert` reads the slot live, and the backend pools repartition on the bumped SVID generation. A `spire` mesh needs **no** `FERRUM_GATEWAY_SVID_*` files; the two are **mutually exclusive** sources for the gateway SVID slot (configuring both fails validation).

Startup is fail-closed: it blocks until the first SVID arrives (bounded by `FERRUM_MESH_CA_BACKEND_READY_TIMEOUT_SECONDS`). On timeout it refuses to start under `FERRUM_MESH_PRODUCTION_MODE=true` (the inbound listener would serve no identity); in dev it warns and continues while the fetch loop keeps retrying and the inbound listener fails closed until an SVID arrives. When `FERRUM_GATEWAY_SPIFFE_ID` is set it pins the identity — an SVID whose SPIFFE ID does not match is rejected, never served.

| Variable | Default | Description |
|---|---|---|
| `FERRUM_MESH_CA_BACKEND` | `none` | CA backend: `none` (no automatic identity), `spire` (SPIRE Workload API — wired; `spire_agent` / `spire-agent` are aliases), `internal` (self-signed dev CA — parsed but not yet wired to issue SVIDs) |
...
| `FERRUM_MESH_CA_BACKEND_READY_TIMEOUT_SECONDS` | `30` | Max seconds startup waits for the first SVID from the `spire` backend before continuing (fail-closed under production, warn-and-continue in dev) |

The SPIRE backend is the recommended — and currently the only wired — production path for mesh identity. `internal` is parsed/validated but does not yet issue runtime SVIDs (a follow-up); it is intended for development and testing only and would generate a self-signed root CA at startup with no external trust anchor.
...
1. **Gateway SVID backs the inbound server identity.** When no explicit `FERRUM_FRONTEND_TLS_CERT_PATH` / `KEY_PATH` is set, the gateway SVID backs the inbound listener's server certificate, so a mesh configured with only `FERRUM_GATEWAY_SVID_*` (or `FERRUM_MESH_CA_BACKEND=spire`) presents that SVID as its inbound server cert and serves mTLS instead of falling open to plaintext under the default PERMISSIVE mode. The inbound server certificate **resolves live from the gateway SVID slot** (a rustls `ResolvesServerCert` backed by the same shared slot the SVID file watcher *and* the SPIRE Workload API fetch loop rotate): after a file-based or SPIRE-driven SVID rotation the inbound listener presents the new leaf on the next handshake, with no restart. In-flight TLS sessions keep their established parameters; only new handshakes see the new leaf. Fail-closed semantics: if a rotation installs material that cannot back a server certificate (or empties the slot), inbound handshakes fail — loudly, once per bad rotation — rather than silently serving a stale leaf that masks a broken rotation pipeline until it expires. Explicit `FERRUM_FRONTEND_TLS_*` material remains a static operational input (the operator owns its rotation; the frontend live-reload flag covers it separately).
...
Wiring the `internal` CA backend (`FERRUM_MESH_CA_BACKEND=internal`) to issue and rotate a runtime SVID into the data plane remains the residual follow-up; the `spire` backend (above) and file-based gateway SVID material both already provide a live-rotating runtime workload identity.
...
| Variable | Default | Description |
|---|---|---|
| `FERRUM_MESH_CA_BACKEND` | `none` | CA backend for mesh SVID issuance: `none`, `spire` (SPIRE Workload API — **wired**, fetches + live-rotates the gateway SVID at startup; `spire_agent` / `spire-agent` are aliases), `internal` (dev-only self-signed root — parsed but not yet wired to issue SVIDs) |
| `FERRUM_MESH_CA_BACKEND_READY_TIMEOUT_SECONDS` | `30` | Max seconds mesh startup waits for the first SVID from the `spire` backend before continuing (fail-closed under production, warn-and-continue in dev) |
...
| `FERRUM_MESH_ALLOW_NO_CA` | `false` | Dev/test opt-in to start `mesh` mode with **no workload identity** (no file-based gateway SVID material and no wired CA backend, i.e. not `FERRUM_MESH_CA_BACKEND=spire`). Without it, an identity-less mesh fails startup closed (no mTLS ⇒ PERMISSIVE accepts plaintext). Read directly from the environment (not `ferrum.conf`); refused unconditionally when `FERRUM_MESH_PRODUCTION_MODE=true`. Lab/test only — see [Internal Dev CA and Production Guardrails](#internal-dev-ca-and-production-guardrails) |
