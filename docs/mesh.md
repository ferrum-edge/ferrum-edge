...
**Transport**: ProxyConfig is not a stock Envoy/Istio xDS resource. On Ferrum-to-Ferrum xDS it rides the dedicated ECDS `ProxyConfigsCarrier` (`ferrum-mesh-carrier/proxy-configs`) with the same slice semantics as native `MeshSubscribe` (pinned by `xds_round_trip_preserves_protected_slice_fields`). Stock Envoy/third-party Istio CPs do not emit that carrier. Separately, the Kubernetes controller does **not** watch `ProxyConfig` CRDs (`ISTIO_CRDS`) and does not write Istio status for them — supply ProxyConfig via native/file mesh config (or any CP path that already has `proxy_configs` in the slice). See [Istio CRD capability dimensions](configuration.md#istio-crd-capability-dimensions).
