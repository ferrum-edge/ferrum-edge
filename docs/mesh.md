...
1. **Gateway/runtime SVID backs the inbound server identity.** When no explicit `FERRUM_FRONTEND_TLS_CERT_PATH` / `KEY_PATH` is set, the mesh workload SVID backs the inbound listener's server certificate. File-based `FERRUM_GATEWAY_SVID_*` material is loaded as the static server identity; CA-backed SVIDs (`spire_agent` / `internal`) use a live rustls server-cert resolver that reads the current SVID slot on every handshake, so inbound server identity follows CA/SPIRE rotation. A mesh configured with only workload SVID material therefore presents that SVID as its inbound server cert and serves mTLS, instead of falling open to plaintext under the default PERMISSIVE mode. **File rotation caveat:** the file-based SVID fallback remains pinned at startup for inbound server identity; the file watcher rotates backend/outbound identity only. For unattended file-based inbound rotation, supply explicit `FERRUM_FRONTEND_TLS_*` with your own rotation, restart/rolling-redeploy on SVID rotation, or use the CA-backed runtime SVID path.
