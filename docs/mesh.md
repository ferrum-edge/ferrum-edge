...
| eBPF ambient capture | **Dev-only** | Requires a build with `--features ebpf`. The **default published image uses a no-op mock backend** (attaches nothing, sets `ferrum_mesh_node_topology_degraded`); a **Linux-only `-ebpf` image variant** (`ferrumedge/ferrum-edge:<tag>-ebpf` / `ghcr.io/ferrum-edge/ferrum-edge:<tag>-ebpf`, built from source with the real aya backend + embedded BPF ELF) is published per release for real capture — it needs a node kernel **≥ 5.7** with cgroup v2 and, on kernel **≥ 5.8**, `CAP_BPF`/`CAP_NET_ADMIN`/`CAP_PERFMON`; on the **5.7.x** window use `CAP_SYS_ADMIN` + `CAP_NET_ADMIN` instead, because `CAP_BPF`/`CAP_PERFMON` were only split out of `CAP_SYS_ADMIN` in 5.8 (see [docs/node_agent_security.md](node_agent_security.md) and [docs/node_agent.md](node_agent.md)). The aya kernel loader compiles on Linux only. CI builds the BPF object (`build-ebpf`), compiles the userspace loader (`build-ebpf-userspace`), and **load/attach-tests the programs on a real ≥5.7 kernel** (`ebpf-live`); the **full capture datapath remains unverified on a live multi-pod node**. **Per-pod inbound redirect (F4.3):** the eBPF TC redirect action is **deferred — it is plan-generation scaffolding, not a working redirect** (the `ferrum_tc_inbound` program returns `TC_ACT_PIPE`, so matching packets pass through the stack rather than to the proxy; a redirect needs a redirect-capable eBPF program). The **functional** per-pod path is a **pod-netns iptables fallback** (a pod-scoped REDIRECT applied inside the pod's own netns, where the pod IP is `LOCAL`); the plan renders correct commands, but *executing* it (entering the pod netns) is not yet wired into the enrollment loop, and live multi-pod verification is a residual. A pod that cannot be scoped per-pod **fails closed** (loud `warn!`, inbound redirect skipped) — it is never widened to the node-global iptables inbound chain. The node-global iptables fallback still needs a custom runtime image with `/bin/sh` + `iptables` (+ `ip6tables` for IPv6). |
...
- **Per-pod inbound redirect (F4.3): TC-eBPF is scaffolding; pod-netns iptables is the functional path** — the host-netns node-global iptables inbound chain cannot scope to one pod (pod IPs are FORWARDED, not `LOCAL`), so F4.3 adds a per-pod `TcRedirectPlan`. The **eBPF TC backend is plan-generation scaffolding, not yet functional**: `ferrum_tc_inbound` returns `TC_ACT_PIPE` (destination rewrite deferred to a redirect-capable program), so its rendered `tc` commands are inert. The **functional** per-pod path is the **pod-netns iptables fallback** (a pod-scoped REDIRECT inside the pod netns, to the HBONE port); its commands are rendered and unit-tested, but entering the pod netns to execute them is a wiring residual, and live multi-pod verification is a residual. **Fail-closed:** a pod that cannot be scoped per-pod gets a loud `warn!` and its inbound redirect is skipped — never widened to node-global capture. The node-global iptables fallback still requires a custom runtime image with `/bin/sh` + `iptables` (+ `ip6tables` for IPv6).
...
- **`connectionPool.http.http1MaxPendingRequests`** — parsed, warned, and surfaced in `status.ferrum.translation.deferred_fields`; not enforced (intended design: a Ferrum-side pending-request gauge).
...
| `trafficPolicy.connectionPool.http.http1MaxPendingRequests` | Ignored (warns + status `deferred_fields`) | Deferred follow-on (the only remaining deferred HTTP knob after F5.1). The translator parses the rule successfully so operators can keep their CRDs unchanged, emits an operator-visible warning, and otherwise drops it. When the Istio controller is active it is also surfaced in the DestinationRule `status.ferrum.translation.deferred_fields` block (see "Istio CRD Status") so the gap shows up in `kubectl describe`. Intended design: a Ferrum-side pending-request gauge on the hot path. |
