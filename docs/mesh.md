...
In addition to remote-cluster gateways, the east-west topology materializes a TCP passthrough proxy for each local `MeshService` in the mesh slice — **one per HTTP-family service port** (multi-port east-west, issue #2010 phase 3; see [Multi-Port Cross-Cluster SNI Aliases](#multi-port-cross-cluster-sni-aliases)). The lowest-numbered HTTP-family port's SNI routing host is the service FQDN (e.g., `reviews.default.svc.cluster.local`); each additional port routes on the deterministic `p<port>.<fqdn>` alias (e.g. `p9090.reviews.default.svc.cluster.local`), each backed by that port's container port — so cross-cluster clients reach every HTTP-family port of a local service through the east-west gateway without separate per-service configuration. An explicit `EastWestGateway.sni_hosts` entry takes precedence over the automatic local-service route for any SNI host it owns (wildcard-aware, matching the same overlap semantics validation uses): the automatic proxy for an overlapping FQDN (base or alias) is suppressed so the explicit route is the one bound on the shared listen port.
...
Local-cluster endpoints stay the first tier (cross-cluster targets carry the reserved `mesh.remote=true` provenance marker, so strict local-first locality LB ranks them after healthy locals). Fail-closed throughout: a missing SNI override, a missing/unparseable remote trust domain, no matching gateway, or no gateway whose `sni_hosts` claims the destination FQDN all skip/refuse the dial (502) — there is no plaintext fallback. **Every HTTP-family service port gets a cross-cluster target** (multi-port east-west, issue #2010 phase 3 — see [Multi-Port Cross-Cluster SNI Aliases](#multi-port-cross-cluster-sni-aliases)): the LOWEST-numbered HTTP-family port routes on the base service FQDN, additional ports on the `p<port>.<fqdn>` alias. The path is HTTP-family only (raw-TCP / UDP cross-cluster egress is not materialized).
...
The HBONE **capability registry is bypassed** for cross-cluster targets (they dial the operator-declared gateway, never a probeable workload `:15008`), and they are excluded from capability collection/probing entirely — the same trust the Sidecar mesh-mTLS path places in operator-declared gateway infra. Local-cluster endpoints stay the first tier (`mesh.remote=true`). Fail-closed throughout: a missing SNI override or a missing/unparseable trust domain refuses the dial (502). **Every HTTP-family service port gets per-remote-pod cross-cluster targets** (multi-port east-west, issue #2010 phase 3): the LOWEST-numbered HTTP-family port routes on the base service FQDN, additional ports on the `p<port>.<fqdn>` alias (the inner HBONE CONNECT `:authority` already carries the per-port app port). The path is HTTP-family only.
...
**gRPC and WebSocket over cross-cluster HBONE (Ambient) are not supported and fail closed.** A native-gRPC request routed to a cross-cluster HBONE target is refused before any dial with a Trailers-Only gRPC UNAVAILABLE (HTTP 200 + `grpc-status: 14`, the gRPC analog of a 502) — the HBONE inner protocol is HTTP/1.1 over a byte tunnel and cannot carry the HTTP/2 trailers gRPC requires, cross-cluster or not (the same limitation as same-cluster Ambient gRPC). WebSocket over cross-cluster HBONE fails the upgrade closed because the HBONE WS byte-tunnel path has no cross-cluster SNI-override / trust-domain-scope plumbing. (PASS-THROUGH gRPC-Web is body-framed and rides the cross-cluster HTTP-family path like plain HTTP; gRPC-Web the `grpc_web` plugin TRANSLATED is wire-native gRPC by dispatch time and is refused pre-dial the same way.) Both are tracked follow-ups (issue #2010). **Sidecar cross-cluster gRPC and WebSocket ARE supported** over mesh-mTLS — see [Client-Side Cross-Cluster Egress (Sidecar)](#client-side-cross-cluster-egress-sidecar) and the [Protocol x Topology Support Matrix](#protocol-x-topology-support-matrix).

### Multi-Port Cross-Cluster SNI Aliases

The east-west gateway routes purely by the ClientHello SNI, which carries a hostname but no port. To reach **every** HTTP-family port of a multi-port service across clusters (issue #2010 phase 3) — not just the first — Ferrum derives a deterministic, DNS-safe per-port SNI from one function (`cross_cluster_service_sni`) that both sides use, so the client's dial SNI and the destination gateway's per-port passthrough proxy cannot drift:

- **Lowest-numbered HTTP-family port** → the base service FQDN `<service>.<namespace>.svc.<cluster-domain>` (e.g. `reviews.default.svc.cluster.local`). Backward-compatible: a single-port service is byte-identical to the pre-multi-port behavior (same SNI, and the internal east-west proxy keeps its port-less id). The base-port choice keys on the port **number**, so it is **order-independent** — two clusters that declare the same service's ports in a different order derive the SAME SNI for each numeric port (a client that dials `p9090.<fqdn>` for port 9090 is matched by a destination that materialized 9090 on that same alias regardless of declaration order).
- **Each other port** → `p<service-port>.<base-fqdn>` (e.g. `p9090.reviews.default.svc.cluster.local`). The **numeric service port** is authoritative in the alias — a port *name* is never used for uniqueness. `p<port>` is a valid DNS label, so the alias is a valid FQDN (no colon-bearing SNI).

**Gateway ownership is automatic (no extra operator config).** The client SELECTS the east-west gateway by the **base** service FQDN (the host the operator's `EastWestGateway.sni_hosts` already claim) but DIALS the per-port alias as the `mesh.eastwest_sni` override. The destination cluster's east-west gateway auto-materializes one SNI-passthrough proxy per HTTP-family service port (base FQDN for the lowest port, `p<port>` alias for the rest), each backed by that port's container port — so a gateway that owns the base FQDN owns its port aliases too. An explicit `EastWestGateway.sni_hosts` entry suppresses the auto local-service proxy for **each SNI it directly claims** (wildcard-aware): a wildcard or fully-enumerated explicit entry owns the whole service, while an entry that lists only the base FQDN suppresses just the base auto proxy and leaves the per-port alias auto proxies materialized — so those ports stay routable to the local workload rather than failing closed (the destination gateway would have no SNI match for an alias the explicit route neither materializes nor lists). A partial explicit override therefore SPLITS the service (base → explicit backend, aliases → local auto route); list the aliases (or a wildcard) in `sni_hosts` to route every port through the explicit backend.

**Fail-closed on skew.** A cross-cluster dial to a per-port alias whose destination gateway never materialized it (version skew: an old destination that predates multi-port) simply finds no SNI match and the connection fails — never a misroute to a different port. Every existing cross-cluster fail-closed gate (missing SNI override, missing/unparseable trust domain, no matching gateway, cross-trust-domain shared endpoint) applies per port unchanged. Both the client materializers and the Sidecar/Ambient **service-discovery bridge** emit per-port targets; only non-HTTP-family ports are skipped (raw-TCP / UDP cross-cluster is a later phase).
...
| WebSocket | **Supported** — RFC 8441 Extended CONNECT over SVID-mTLS to `:15006`; fail-closed, never plaintext [6] | **Supported** — bare HBONE CONNECT byte tunnel + inner HTTP/1.1 upgrade through the tunnel [6] | **Supported** — RFC 8441 Extended CONNECT over the cross-cluster mesh-mTLS dial (shared `MeshMtlsDialPlan`: east-west gateway dial + destination-FQDN SNI override + trust-domain-only verification) across **all HTTP-family service ports** (per-port SNI alias, phase 3); fail-closed on missing SNI/trust-domain or a failed upgrade, never a plaintext/wrong-SNI dial [7] | **Fail-closed** — cross-cluster Ambient WebSocket upgrade fails closed (no SNI-override / trust-domain scope on the HBONE WS byte-tunnel path) [7] | **Supported** — the WebSocket-over-mesh dispatch keys on the same `mesh.mtls`/`mesh.hbone` target tags; a Sidecar SD-bridged east-west failover target now upgrades through the east-west gateway (cross-cluster Sidecar), while an Ambient one refuses like the cross-cluster Ambient column [6] [7] |
...
7. Cross-cluster WebSocket: **Sidecar is supported** — the upgrade opens an RFC 8441 Extended CONNECT over the cross-cluster mesh-mTLS dial resolved by the shared `MeshMtlsDialPlan` (east-west gateway dial + `mesh.eastwest_sni` SNI override + trust-domain-only verification), the WebSocket analogue of the gRPC/HTTP cross-cluster path; see [Client-Side Cross-Cluster Egress (Sidecar)](#client-side-cross-cluster-egress-sidecar) (issue #2010). **Ambient stays fail-closed** — the HBONE WS byte-tunnel path has no cross-cluster SNI-override / trust-domain scope; see [Client-Side Cross-Cluster Egress (Ambient / HBONE)](#client-side-cross-cluster-egress-ambient--hbone).
