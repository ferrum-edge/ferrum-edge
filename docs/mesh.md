...
- **HTTP/1.1** — an **exclusive** lease. The connection re-enters the idle set only after the **entire** response body has been read, which is the buffered-response path. A streaming, upgraded, errored, partial, or `Connection: close` response never returns its lease: it is dropped, which closes the connection. Receiving response headers is never sufficient, and neither is hyper's own `SendRequest::ready()` — h1 `can_write_head()` is already true while a response body is still being read, so readiness would re-pool a connection mid-body and pipeline the next request onto it. **Pooling streaming HTTP/1.1 responses therefore remains a follow-up**; those responses still take a dedicated connection, exactly as before.
...
Bounds and lifecycle: the idle set per identity is capped by the global `max_idle_per_host`, entries expire against the effective `idle_timeout_seconds`, and an amortized sweep (at most once per interval, process-wide) evicts closed, expired, and identity-changed entries. The pool is owned by `ProxyState`, which is rebuilt wholesale on config replacement — so a reload constructs a new pool and drops the old one, ending each connection's driver task when its last sender drops. Target withdrawal, a changed UID/path/protocol, and a namespace/proxy change all yield a different key, so a connection admitted under an old identity is never handed out under a new one. Physical connects, hits, misses, identity retirements, setup failures, and idle/active counts are exported as bounded counters with no per-target labels.
