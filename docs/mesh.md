...
2. **Refuse a would-be-plaintext inbound listener.** If the resolved inbound mTLS/HBONE termination listener would still come up plaintext — PeerAuthentication resolved to DISABLE, or no usable server identity — or its configured SPIFFE verifier failed to load, startup is **refused**: unconditionally under `FERRUM_MESH_PRODUCTION_MODE=true`, and in dev unless `FERRUM_MESH_ALLOW_NO_CA=true` (which then logs a loud warning and continues). Topologies with no TLS-terminating inbound listener (EastWestGateway SNI passthrough forwards encrypted bytes) are exempt.
