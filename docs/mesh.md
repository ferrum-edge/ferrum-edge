...
- **Node-waypoint destination metadata** — each mesh `Workload` can carry an optional `node_waypoint` object with `address`, `hbone_port` (default `15008`), expected NodeWaypoint `spiffe_id`, and optional `node_name`, `node_uid`, `network`, and `cluster`. This is the machine-readable contract the secured NodeWaypoint transport will consume: select the workload first, dial the hosting node's NodeWaypoint endpoint over SPIFFE-mTLS HBONE, and pin the endpoint SPIFFE ID instead of synthesizing `podIP:15008`.
