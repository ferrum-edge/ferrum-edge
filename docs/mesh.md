...
- `ferrum_mesh_bpf_srtt_microseconds_{sum,count}`, `ferrum_mesh_bpf_syn_to_ack_microseconds_{sum,count}` — TCP-layer latency aggregates. Operators derive averages from `sum / count`. App-layer latency stays in `workload_metrics`. Accept-to-first-byte is **not** exported: SOCK_OPS has no first-inbound-data-byte callback, so a zero summary would advertise an unsupported contract.
...
Set `FERRUM_MESH_CLUSTER_AUDIENCE` on each cluster's control plane to the identifier its peers use in `RemoteCluster.name`. Both values are identifiers, never credentials: neither is a secret, and neither is derived from one.
...
| `remote_discovery = false` (ordinary local mesh DP) | none — this class carries no `aud` | **any** `aud` at all; a reserved `ferrum-mesh-discovery:` value is reported distinctly (`reserved_audience`) from an unrelated one (`unexpected_audience`) |
...
- Cross-cluster discovery tokens carry the reserved `ferrum-mesh-discovery:` audience prefix. Ordinary CP↔DP `ConfigSync`, xDS ADS, and local mesh `MeshSubscribe` all **refuse** a token carrying it — a discovery token cannot be replayed as a local subscription token, including by clearing the `remote_discovery` flag.
- Ordinary CP↔DP / local mesh tokens carry **no** `aud` at all (their shape is unchanged). They therefore can never satisfy the discovery policy, which requires a present, matching audience. Conversely, those surfaces accept **no** audience: this preserves `jsonwebtoken`'s strict `validate_aud` posture (RFC 7519 §4.1.3), so a token minted for some other service cannot authenticate to the CP/DP plane merely because the HS256 secret is shared. Operator-minted CP↔DP tokens must therefore not stamp an `aud`.

**Layering.** Audience binding is defense in depth **on top of** — never instead of — the existing checks: HS256 signature against the per-remote (or shared) secret, `exp`/`iat`, pinned issuer, `role`, the `ns` namespace claim, TLS/production transport posture, and the fail-closed per-remote credential resolution all still apply unchanged, and a failure in any of them still refuses the subscription.

**Failure handling and diagnostics.** A refused subscription fails the whole poll: the data plane keeps that cluster's **last-good endpoints**, backs off, and imports nothing (the existing `FERRUM_MESH_REMOTE_DISCOVERY_MAX_STALE_SECONDS` window still governs eventual withdrawal). The control plane records the refusal as `ferrum_mesh_subscribe_audience_rejections_total{subscription,reason}` — `subscription` is `remote_discovery` or `local`, `reason` is one of `missing`, `malformed`, `ambiguous`, `mismatch`, `unconfigured`, `reserved_audience`, `unexpected_audience` — and emits an `audit.event="mesh_subscribe_audience_rejected"` `WARN`. Both labels are compile-time constants, so the series cardinality is fixed. The token, its claims, the expected audience, and the JWT secret are never logged or exported; the gRPC status returned to the caller is a fixed string that does not echo the presented audience.
