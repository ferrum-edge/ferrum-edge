...
| UDP | **Supported (Experimental)** — TPROXY capture → `udp`-marked mesh CONNECT over mesh-mTLS; unroutable datagrams dropped fail-closed [10] | **Source-capture + transport only** — per-pod-netns TPROXY producer captures source UDP inside each enrolled pod's netns and relays it over HBONE (#2013); unroutable datagrams dropped fail-closed. The full e2e relay to an **enrolled** Ambient destination pod (return path included) is **not yet functional** — see [10] [12] | **Not materialized** [2] | **Not materialized** [3] | **Not part of the bridge** [9] |
| DTLS-over-UDP | **Supported (Experimental), opaque** — a DTLS port is declared `protocol: UDP` and rides the same UDP datapath byte-for-byte; the mesh never terminates the DTLS session [11] | **Source-capture + transport only, opaque** — rides the Ambient per-pod-netns UDP producer (source capture + HBONE transport), inheriting its e2e limitation to an enrolled destination pod [10] [11] [12]; DTLS never terminated | **Not materialized** [2] | **Not materialized** [3] | **Not part of the bridge** [9] |
...
10. Raw-TCP / UDP egress bullet under [Topologies → Sidecar](#sidecar) plus the "UDP TPROXY capture" section under Capture Modes: UDP egress is dual-transport (Ambient over HBONE `:15008`, Sidecar over mesh-mTLS `:15006`). Both topologies now have a working source-capture producer: **Sidecar** via the injector's pod-netns TPROXY init rules + the current-netns capture listener; **Ambient** via the per-pod-netns producer (`NetnsUdpCaptureManager`, #2013) — the Ambient proxy runs outside the pod netns and the host-netns iptables fallback emits no UDP rules, so the producer installs the UDP TPROXY rules and binds the transparent capture + reply sockets INSIDE each enrolled pod's netns (`setns`), reusing the exact injector rule shapes and the shared capture/egress loop. The Ambient producer rides the same per-pod registry the node-agent publishes for NodeWaypoint (now also published for Ambient when UDP capture is enabled). **Scoped-policy limitation:** Ambient UDP source-capture does not carry per-pod policy evidence, so captured UDP egress is authorized mesh-wide (the datagram is relayed under the gateway's mesh identity) — do not claim per-pod scoped UDP authorization on this path. Residual: the reconcile / rule-generation / registry logic is unit-tested, but the full **live source-capture e2e** (TPROXY → tunnel → return-source on a transparent socket) still rides the `netns-capture-live` follow-up (issue #2013) for both topologies. The `functional_mesh_udp_dest_*` tests exercise the destination relay **in isolation** — a host-netns relay socket delivering to a plain (non-enrolled) local UDP listener; they do **not** cover the destination pod being itself an **enrolled** Ambient pod, where the TC-inbound eBPF guard and the pod's own OUTPUT-mark rules are active. See [12] for the end-to-end Ambient relay gap and why it is not yet functional.
...
12. **End-to-end Ambient UDP relay to an enrolled destination pod is not yet functional** (tracked under #2013). The per-pod-netns producer (#2030) delivers the **source** half — capture inside the enrolled pod's netns + HBONE datagram transport — but two datapath pieces are missing when the *destination* is also an enrolled Ambient pod, and both fail **closed** (the datagram is dropped, not leaked in plaintext — a completeness gap, not a security regression): (a) the destination pod's `ferrum_tc_inbound` eBPF program (`ebpf/ferrum-ebpf/src/tc_inbound.rs`) DROPS UDP to an enrolled pod IP except DNS, so the relay's unmarked host-netns socket cannot deliver — this needs a TC-inbound UDP admit keyed on the relay socket's mark/identity; and (b) the destination-side inbound relay socket binds in the **host** netns (`hbone_proxy.rs` `handle_hbone_udp_request`), so the app's reply is non-LOCAL from the pod netns and is re-captured by the pod's own OUTPUT-mark → TPROXY path — the structurally correct fix binds the dest-side inbound relay socket **inside** the destination pod netns (mirroring the NodeWaypoint TCP in-netns `127.0.0.1:<port>` listener) so the reply is pod-LOCAL and already excluded by the existing `! --dst-type LOCAL` scope. Both require a live two-pod (both enrolled) `netns-capture-live` fixture to verify and are out of scope for the transport-half producer (#2030).
...
- **End-to-end status (#1808, #2013): both topologies now have a UDP source-capture
  ...
  **Scoped-policy
  ...
  live source-capture e2e rides the `netns-capture-live` follow-up for both
  topologies. eBPF UDP capture stays a non-goal (#1803 per-pod TC not resurrected).
