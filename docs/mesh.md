...
In addition to remote-cluster gateways, the east-west topology materializes a TCP passthrough proxy for each local `MeshService` in the mesh slice — **one per HTTP-family service port** (multi-port east-west, issue #2010 phase 3; see [Multi-Port Cross-Cluster SNI Aliases](#multi-port-cross-cluster-sni-aliases)). A **single-HTTP-family-port** service's SNI routing host is the base service FQDN (e.g., `reviews.default.svc.cluster.local`), backed by that port's container port. A **multi-HTTP-family-port** service materializes an explicit `p<port>.<fqdn>` alias proxy for **every** port — the numerically lowest included (e.g. `p8080.reviews.default.svc.cluster.local`, `p9090.reviews.default.svc.cluster.local`) — each backed by its own container port; no auto proxy claims the bare base FQDN for a multi-port service. So cross-cluster clients reach every HTTP-family port of a local service through the east-west gateway without separate per-service configuration. An explicit `EastWestGateway.sni_hosts` entry takes precedence over the automatic local-service route for any SNI host it owns (wildcard-aware, matching the same overlap semantics validation uses): the automatic proxy for an overlapping FQDN (base or alias) is suppressed so the explicit route is the one bound on the shared listen port. An explicit entry that lists only the base FQDN of a multi-port service suppresses nothing (there is no base auto proxy for a multi-port service) and leaves every per-port alias proxy materialized.
...
Local-cluster endpoints stay the first tier (cross-cluster targets carry the reserved `mesh.remote=true` provenance marker, so strict local-first locality LB ranks them after healthy locals). Fail-closed throughout: a missing SNI override, a missing/unparseable remote trust domain, no matching gateway, or no gateway whose `sni_hosts` claims the destination FQDN all skip/refuse the dial (502) — there is no plaintext fallback. **Every HTTP-family service port gets a cross-cluster target** (multi-port east-west, issue #2010 phase 3 — see [Multi-Port Cross-Cluster SNI Aliases](#multi-port-cross-cluster-sni-aliases)): a single-HTTP-port service routes on the base service FQDN; a multi-HTTP-port service routes **every** port on its explicit `p<port>.<fqdn>` alias (the lowest port included — no port maps onto the bare base FQDN). The path is HTTP-family only (raw-TCP / UDP cross-cluster egress is not materialized).
...
The HBONE **capability registry is bypassed** for cross-cluster targets (they dial the operator-declared gateway, never a probeable workload `:15008`), and they are excluded from capability collection/probing entirely — the same trust the Sidecar mesh-mTLS path places in operator-declared gateway infra. Local-cluster endpoints stay the first tier (`mesh.remote=true`). Fail-closed throughout: a missing SNI override or a missing/unparseable trust domain refuses the dial (502). **Every HTTP-family service port gets per-remote-pod cross-cluster targets** (multi-port east-west, issue #2010 phase 3): a single-HTTP-port service routes on the base service FQDN; a multi-HTTP-port service routes **every** port on its explicit `p<port>.<fqdn>` alias (the lowest port included — no port maps onto the bare base FQDN), and the inner HBONE CONNECT `:authority` already carries the per-port app port. The path is HTTP-family only.
...
The east-west gateway routes purely by the ClientHello SNI, which carries a hostname but no port. To reach **every** HTTP-family port of a multi-port service across clusters (issue #2010 phase 3) — not just the first — Ferrum derives a deterministic, DNS-safe SNI from one function (`cross_cluster_service_sni`) that both sides use, so the client's dial SNI and the destination gateway's per-port passthrough proxy cannot drift. The SNI depends only on whether the service has **one** HTTP-family port or **several**:
...
**Gateway ownership is automatic (no extra operator config).** The client DIALS the per-port alias (single-port: the base FQDN) as the `mesh.eastwest_sni` override, and SELECTS an east-west gateway whose `sni_hosts` claim **either** the base service FQDN (whose ownership auto-extends to the service's per-port aliases) **or** the exact per-port alias being dialed (codex #2040 Finding C — an operator may list just one port's alias). The destination cluster's east-west gateway auto-materializes one SNI-passthrough proxy per HTTP-family service port (base FQDN for a single-port service; a `p<port>` alias for every port of a multi-port service), each backed by that port's container port — so a gateway that owns the base FQDN owns its port aliases too. An explicit `EastWestGateway.sni_hosts` entry suppresses the auto local-service proxy for **each SNI it directly claims** (wildcard-aware): a wildcard or fully-enumerated explicit entry owns the whole service, while an entry that lists only the base FQDN of a multi-port service suppresses nothing (there is no base auto proxy for a multi-port service) and leaves every per-port alias auto proxy materialized — so those ports stay routable to the local workload rather than failing closed. Adding an explicit gateway for specific per-port aliases (or a wildcard) routes those ports through the explicit backend.

**Fail-closed on skew.** A cross-cluster dial to a per-port alias whose destination gateway never materialized it (version skew: an old destination that predates multi-port, or a peer whose port set differs) simply finds no SNI match and the connection fails — never a misroute to a different port. Every existing cross-cluster fail-closed gate (missing SNI override, missing/unparseable trust domain, no matching gateway, cross-trust-domain shared endpoint) applies per port unchanged. Both the client materializers and the Sidecar/Ambient **service-discovery bridge** emit per-port targets; only non-HTTP-family ports are skipped (raw-TCP / UDP cross-cluster is a later phase).
...
2. [Client-Side Cross-Cluster Egress (Sidecar)](#client-side-cross-cluster-egress-sidecar): the east-west gateway routes by SNI, and **every HTTP-family service port** gets a cross-cluster target via the per-port SNI alias scheme (issue #2010 phase 3; see [Multi-Port Cross-Cluster SNI Aliases](#multi-port-cross-cluster-sni-aliases)) — a single-HTTP-port service on the base FQDN; a multi-HTTP-port service on an explicit `p<port>.<fqdn>` alias for every port (the lowest included — no port on the bare base FQDN). The path is HTTP-family only — raw-TCP / UDP cross-cluster egress is not materialized. Fail-closed throughout; no plaintext fallback.
