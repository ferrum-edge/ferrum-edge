...
- **Config publication / reload.** Every publication carrying a resource delta hands the pool the exact set of `mesh.unix_socket` target identities the new config declares — `(namespace, proxy id, upstream id, configured socket path, wire protocol)`. Anything pooled outside that set is retired in one pass before it can serve another request. That covers a withdrawn target, a deleted proxy or upstream, a proxy re-bound to a different namespace or upstream, a re-pointed socket path, and an `http` ⇄ `http2` flip. Comparison is exact tuple equality — never a substring, prefix, or path-containment rule.
- **The withdrawal fence.** Retiring the idle sets is only half of it: an exclusive HTTP/1.1 lease that is checked out is *not* in the idle set, so publication cannot see it, and its exchange can finish long after its target was withdrawn. A still-open, still-same-inode connection expresses nothing about config, so a late check-in would otherwise repopulate the pool under an identity that no longer exists. Every retirement therefore also advances a monotonic **publication generation**, each lease records the generation it was taken under, and a check-in is admitted only while that generation is still current. This is strictly stronger than re-testing set membership — it costs one atomic load rather than a per-request identity lookup, and it also closes same-key ABA: a target withdrawn and re-added under the identical tuple is a new incarnation, and a lease from the previous one may not re-enter it. The cost is that a lease outstanding across an unrelated publication is retired instead of reused (one connection per in-flight exchange per publication). The fence is evaluated twice, around the insert, and publication bumps the generation *before* it retires, so the racing order that defeats a single check — read the generation, publication bumps and retires, then insert — is caught by the second read, which withdraws exactly the entry it inserted. The same fence guards the shared h2c carrier published at the end of a dial.
