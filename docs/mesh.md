...
- **Graceful shutdown.** Each serving mode drains the pool from inside its existing bounded shutdown sequence — after accept loops stop and after in-flight requests have had their `FERRUM_SHUTDOWN_DRAIN_SECONDS` budget — so the drain neither shortens nor is shortened by that window. The drain also latches the pool closed, so a streaming response finishing in the last moments of the drain retires its carrier instead of re-pooling it.
