...
| UDP | **Supported (Experimental)** — TPROXY capture → `udp`-marked mesh CONNECT over mesh-mTLS; unroutable datagrams dropped fail-closed [10] | **Supported (Experimental)** — per-pod-netns TPROXY producer captures source UDP inside each enrolled pod's netns and relays it over HBONE; enrolled destination pod replies use a destination pod-netns relay socket when the registry maps the destination IP [10] [12] | **Not materialized** [2] | **Not materialized** [3] | **Not part of the bridge** [9] |
| DTLS-over-UDP | **Supported (Experimental), opaque** — a DTLS port is declared `protocol: UDP` and rides the same UDP datapath byte-for-byte; the mesh never terminates the DTLS session [11] | **Supported (Experimental), opaque** — rides the Ambient per-pod-netns UDP producer and destination pod-netns relay socket where applicable; DTLS is framed byte-for-byte and never terminated [10] [11] [12] | **Not materialized** [2] | **Not materialized** [3] | **Not part of the bridge** [9] |
...
10. Raw-TCP / UDP egress bullet under [Topologies → Sidecar](#sidecar) plus the "UDP TPROXY capture" section under Capture Modes: UDP egress is dual-transport (Ambient over HBONE `:15008`, Sidecar over mesh-mTLS `:15006`). Both topologies now have a working source-capture producer: **Sidecar** via the injector's pod-netns TPROXY init rules + the current-netns capture listener; **Ambient** via the per-pod-netns producer (`NetnsUdpCaptureManager`, #2013) — the Ambient proxy runs outside the pod netns and the host-netns iptables fallback emits no UDP rules, so the producer installs the UDP TPROXY rules and binds the transparent capture + reply sockets INSIDE each enrolled pod's netns (`setns`), reusing the exact injector rule shapes and the shared capture/egress loop. The Ambient producer rides the same per-pod registry the node-agent publishes for NodeWaypoint. When UDP capture is disabled, mesh startup still runs a best-effort cleanup manager over that registry to remove stale Ferrum UDP rules from a prior enabled run. **Scoped-policy limitation:** Ambient UDP source-capture does not carry per-pod policy evidence, so captured UDP egress is authorized mesh-wide (the datagram is relayed under the gateway's mesh identity) — do not claim per-pod scoped UDP authorization on this path. Residual: the reconcile / rule-generation / registry logic is unit-tested, but the full **live source-capture e2e** (TPROXY → tunnel → return-source on a transparent socket) still rides the `netns-capture-live` follow-up (issue #2013) for both topologies. The `functional_mesh_udp_dest_*` tests exercise the destination relay against a plain local UDP listener; enrolled Ambient destination pod socket placement is unit-pinned and awaits the same live two-pod netns fixture. See [12].
...
- **End-to-end status (#1808, #2013): both topologies now have a UDP source-capture
  ...
  **Scoped-policy
  ...
  rule-generation / registry logic is unit-tested; the full live source-capture e2e
  rides the `netns-capture-live` follow-up for both topologies. eBPF UDP capture
  stays a non-goal (#1803 per-pod TC not resurrected).
