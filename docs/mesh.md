...
- **Ingress is materialized for both.** Sidecar inbound builds loopback routes (`:15006` → `127.0.0.1:<appPort>`); Ambient/Waypoint inbound is **transparent** — the HBONE relay dials the CONNECT `:authority` directly, so it needs no route materialization.
- **Egress:** **Ambient outbound** now materializes per-service HBONE routes (a host-routed `/` proxy per in-mesh Service → `mesh.hbone`-tagged upstreams dialing the destination's `:15008`, over the already-built HBONE transport). **Sidecar outbound** (SVID-mTLS HTTP to a peer's `:15006`) is a *distinct transport* being added next — until then Sidecar outbound requires explicitly supplied proxies. Original-destination routing for multi-port / TCP egress (and lifting the single-HTTP-port fail-closed guard) is a later stage.
  - **Known gap:** these materialized outbound upstreams are static-target, so per-port `DestinationRule.trafficPolicy.portLevelSettings` reach them only when the dial port equals the service port — a service whose `targetPort` differs from its `port` drops its *per-port* DR settings (top-level `connectTimeout`/`loadBalancer`/`outlierDetection` still apply). Same limitation as east-west static-target upstreams; it resolves when outbound upstreams move to service-discovery-backed targets (planned with the Sidecar-egress work).

An outbound `404` for an un-materialized destination means no route was built for it — not that mTLS/HBONE is unavailable.
