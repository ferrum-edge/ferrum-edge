...
**L4 test boundary.** In-tree projection tests cover Sidecar/Ambient raw-TCP and UDP target materialization, per-port SNI generation, and destination east-west SNI relay creation. Existing functional tests cover the raw byte-stream and framed-datagram destination relays. A full source-capture run — `SO_ORIGINAL_DST`/TPROXY in cluster A → east-west gateway → captured terminator/app in cluster B, including UDP return-source spoofing and Ambient pod-netns placement — still requires the privileged live two-cluster/netns fixture; it is not simulated by the unprivileged process harness.
