...
The admission check catches static retry/mesh conflicts early and converts a silent 502 into an actionable config error. Operators who need retry on a mesh destination must drop the conflicting `Proxy.retry` (or scope it to a non-mesh subset).

As dispatch-time defense-in-depth, both the generic HTTP retry loop and the gRPC retry loop re-screen the selected target after any load-balancer retry rotation. If a mixed upstream rotates from a non-mesh target onto `mesh.hbone`, `mesh.mtls`, or `mesh.cross_cluster`, the retry fails closed before any direct dial; the synthetic refusal is classified `dispatch_policy_rejected`, so it is neutral to backend circuit breakers and passive health.
