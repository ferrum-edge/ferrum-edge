...
`Sidecar.outboundTrafficPolicy.mode` is **not** gated by `FERRUM_MESH_SIDECAR_ENFORCED`: once translated, the applicable Sidecar's mode (when set) overrides mesh-wide `MeshConfig.outbound_traffic_policy` / `FERRUM_MESH_OUTBOUND_TRAFFIC_POLICY` for that workload's slice. Egress host narrowing remains gated; the outbound-policy security control is not.
...
The same Sidecar selection tiers choose `outboundTrafficPolicy`. The selected Sidecar's own `outboundTrafficPolicy` is used; Ferrum does **not** walk the egress `inherits_defaults` chain for the policy field (an ingress-only workload Sidecar that inherits egress hosts from a namespace default still keeps its own outbound mode when set).

### Outbound traffic policy

Istio `spec.outboundTrafficPolicy.mode` on a `Sidecar`:

| Source shape | Ferrum behavior |
|---|---|
| Field omitted / JSON `null` | Inherit mesh-wide `MeshConfig.outbound_traffic_policy`, else `FERRUM_MESH_OUTBOUND_TRAFFIC_POLICY` (default `allow_any`) |
| `mode: ALLOW_ANY` / `allow_any` (ASCII case-insensitive) | Explicit passthrough contract: no `mesh_outbound_registry` plugin; stream capture paths do not install the REGISTRY_ONLY gate. Unrelated authz / mTLS / TLS policy is unchanged |
| `mode: REGISTRY_ONLY` / `registry_only` | Fail-closed egress: HTTP-family unknown destinations rejected via auto-injected `mesh_outbound_registry` + outbound-capture route-miss gate; TCP/TLS graceful close and UDP/DTLS silent datagram drop on capture listeners; registry includes service hosts, ServiceEntry hosts (incl. wildcards), and workload addresses/IPs so unknown IP/DNS-resolved destinations cannot escape by protocol family |
| Object present with missing / empty / non-string / unsupported `mode` | Hard reject (`FerrumAccepted=False` / `Invalid`) with a field-specific diagnostic — Ferrum does not adopt the protobuf zero-value (`REGISTRY_ONLY`) for an incomplete declaration |

Create / update / delete / reload: translating a Sidecar with a mode projects it onto the next slice; removing the Sidecar or clearing the field restores mesh-wide / runtime inheritance; xDS carries the effective value on `OutboundTrafficPolicyCarrier` (Sidecar records themselves do not ride the slice). Status reports the mode under `status.ferrum.translation.outbound_traffic_policy` and never lists a successfully translated mode in `deferred_fields`.

DNS: the mesh DNS proxy continues to answer from the slice-derived table and may forward non-mesh names upstream for resolution only; under `REGISTRY_ONLY`, subsequent dials to unknown IPs/hosts remain fail-closed on the HTTP and stream capture paths (the registry admits known workload addresses so IP literals cannot bypass hostname checks).
...
| `FERRUM_MESH_OUTBOUND_TRAFFIC_POLICY` | `allow_any` | Mesh-wide outbound policy fallback: `allow_any` or `registry_only`. Overridden by native `MeshConfig.outbound_traffic_policy` and by an applicable Sidecar's `outboundTrafficPolicy.mode` when set |
