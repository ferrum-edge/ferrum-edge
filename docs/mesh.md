...
As dispatch-time defense-in-depth, every direct-dial retry loop — the generic HTTP retry loop, the gRPC retry loops, and the H3 plain-bridge and H3 WebSocket connect loops — re-screens the selected target after any load-balancer retry rotation. If a mixed upstream rotates from a non-mesh target onto `mesh.hbone`, `mesh.mtls`, or `mesh.cross_cluster`, the retry fails closed before any direct dial; the synthetic refusal is classified `dispatch_policy_rejected`, so it is neutral to backend circuit breakers and passive health.
