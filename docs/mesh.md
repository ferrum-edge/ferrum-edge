...
**Runtime inbound mTLS fail-closed (the robust complement).** The gate above is a fast config-time *presence* check; it cannot see whether the SVID files actually load or whether the resolved PeerAuthentication mode would still leave the inbound listener serving plaintext. The mesh therefore also fails closed at the runtime TLS-setup path, where the inbound listener's real posture is known (`enforce_mesh_inbound_fail_closed`). Two refinements make this exact:

1. **Gateway SVID backs the inbound server identity.** When no explicit `FERRUM_FRONTEND_TLS_CERT_PATH` / `KEY_PATH` is set, the gateway SVID's leaf + key are loaded as the inbound listener's server certificate. A mesh configured with only `FERRUM_GATEWAY_SVID_*` therefore presents that SVID as its inbound server cert and serves mTLS, instead of falling open to plaintext under the default PERMISSIVE mode (previously the SVID backed only the *peer verifier*, not the server cert).
2. **Refuse a would-be-plaintext inbound listener.** If the resolved inbound mTLS/HBONE termination listener would still come up plaintext — PeerAuthentication resolved to DISABLE, or no usable server identity — or its configured SPIFFE verifier failed to load, startup is **refused under `FERRUM_MESH_PRODUCTION_MODE=true`** (a production mesh must serve mTLS on its inbound listener). In dev it is allowed with a loud warning: reaching a plaintext posture in dev is intentional — either an explicit `PeerAuthentication` DISABLE, or a no-identity posture the config-time `FERRUM_MESH_ALLOW_NO_CA` gate already let through — so the runtime gate does not re-refuse it. The **same enforcement runs on PeerAuthentication live reload** (`FERRUM_MESH_PEER_AUTH_LIVE_RELOAD_ENABLED=true`): a later accepted update that would downgrade the inbound listener to plaintext is *rejected* under production (the slice is kept-last-good in its entirety — fail-closed by retention, never crashing a running data plane), so a production sidecar cannot start serving plaintext after startup. Topologies with no TLS-terminating inbound listener (EastWestGateway SNI passthrough forwards encrypted bytes) are exempt.
