...
Local-cluster endpoints stay the first tier (cross-cluster targets carry the reserved `mesh.remote=true` provenance marker, so strict local-first locality LB ranks them after healthy locals). Fail-closed throughout: a missing SNI override, a missing/unparseable remote trust domain, no matching gateway, or no gateway whose `sni_hosts` claims the destination FQDN all skip/refuse the dial (502) — there is no plaintext fallback. **Every HTTP-family service port gets a cross-cluster target** (multi-port east-west, issue #2010 phase 3 — see [Multi-Port Cross-Cluster SNI Aliases](#multi-port-cross-cluster-sni-aliases)): the FIRST declared port routes on the base service FQDN, additional ports on the `p<port>.<fqdn>` alias. The path is HTTP-family only (raw-TCP / UDP cross-cluster egress is not materialized).
...
The HBONE **capability registry is bypassed** for cross-cluster targets (they dial the operator-declared gateway, never a probeable workload `:15008`), and they are excluded from capability collection/probing entirely — the same trust the Sidecar mesh-mTLS path places in operator-declared gateway infra. Local-cluster endpoints stay the first tier (`mesh.remote=true`). Fail-closed throughout: a missing SNI override or a missing/unparseable trust domain refuses the dial (502). **Every HTTP-family service port gets per-remote-pod cross-cluster targets** (multi-port east-west, issue #2010 phase 3): the FIRST declared port routes on the base service FQDN, additional ports on the `p<port>.<fqdn>` alias (the inner HBONE CONNECT `:authority` already carries the per-port app port). The path is HTTP-family only.
...
- **First declared port** → the base service FQDN `<service>.<namespace>.svc.<cluster-domain>` (e.g. `reviews.default.svc.cluster.local`). Backward-compatible: a single-port service is byte-identical to the pre-multi-port behavior.
- **Each additional port** → `p<service-port>.<base-fqdn>` (e.g. `p9090.reviews.default.svc.cluster.local`). The **numeric service port** is authoritative in the alias — a port *name* is never used for uniqueness. `p<port>` is a valid DNS label, so the alias is a valid FQDN (no colon-bearing SNI).

**Gateway ownership is automatic (no extra operator config).** The client SELECTS the east-west gateway by the **base** service FQDN (the host the operator's `EastWestGateway.sni_hosts` already claim) but DIALS the per-port alias as the `mesh.eastwest_sni` override. The destination cluster's east-west gateway auto-materializes one SNI-passthrough proxy per HTTP-family service port (base FQDN for the first, `p<port>` alias for the rest), each backed by that port's container port — so a gateway that owns the base FQDN owns its port aliases too. An explicit `EastWestGateway.sni_hosts` entry still takes precedence over an auto-materialized alias it overlaps (wildcard-aware).
