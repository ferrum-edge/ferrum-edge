...
| gRPC | **Not routed over the mesh transport** — the gRPC dispatch branch runs before the HBONE / mesh-mTLS dispatch gates and dials the selected target's `host:port` directly via the gRPC connection pool [5] | **Not routed over the mesh transport** — same direct-dial gRPC branch [5] | **Fail-closed** — refused before any dial with gRPC UNAVAILABLE (the 502 analog); must not bypass the tunnel [5] | **Fail-closed** — same guard [5] | **Not routed over the mesh transport** — same direct-dial gRPC branch [5] |
...
5. Cross-cluster: the fail-closed guard documented at [Client-Side Cross-Cluster Egress (Ambient / HBONE)](#client-side-cross-cluster-egress-ambient--hbone) ("gRPC and WebSocket over cross-cluster HBONE are not yet supported and fail closed"); gRPC-over-HBONE / gRPC-over-mesh-mTLS cross-cluster is a tracked follow-up (HTTP-first). Same-cluster / SD bridge: per the dispatch order in `src/proxy/mod.rs`, the gRPC branch runs **before** the HBONE / mesh-mTLS dispatch gates and dials `target.host:target.port` directly via `GrpcConnectionPool` (h2c for `http`, TLS+ALPN=h2 for `https`), so a gRPC-flavored request to a `mesh.mtls`/`mesh.hbone`-tagged destination does not ride `:15006`/`:15008`.
...
7. Cross-cluster WebSocket fail-closed paragraphs in [Client-Side Cross-Cluster Egress (Sidecar)](#client-side-cross-cluster-egress-sidecar) and [Client-Side Cross-Cluster Egress (Ambient / HBONE)](#client-side-cross-cluster-egress-ambient--hbone); tracked follow-ups (HTTP-first).
