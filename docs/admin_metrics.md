...
| `/metrics` | GET | JWT / metrics token / allowed CIDR | 5-second TTL (configurable via `render_cache_ttl_seconds`) | Prometheus exposition format, including request and TLS certificate gauges |
