...
- **`/metrics`** — Prometheus text format with per-proxy request counters, latency histograms, TLS certificate inventory gauges (`ferrum_tls_cert_expiry_seconds`, `ferrum_tls_cert_not_before_seconds`), certificate rotation counters (`ferrum_tls_cert_rotations_total`), TLS source watcher outcomes (`ferrum_tls_source_refresh_total`), source fetch duration histograms (`ferrum_tls_source_fetch_duration_seconds`), source fetch failure counters (`ferrum_tls_source_fetch_failures_total`), bounded database rejected-delta metrics (`ferrum_database_delta_rejections_total`, `ferrum_database_delta_backoff_bucket`, `ferrum_database_delta_forced_full_reloads_total`, `ferrum_database_delta_recoveries_total`), and admin connection-limiter metrics (`ferrum_admin_active_connections`, `ferrum_admin_max_connections`, `ferrum_admin_rejected_connections_total{reason}`). Requires the `prometheus_metrics` plugin to be enabled. Best for Prometheus/Grafana scraping.
- **`/health`** — Lightweight health probe for load balancers and orchestrators (Kubernetes liveness/readiness). In database mode, `database_polling.status: "degraded"` identifies a repeatedly rejected incremental delta while the gateway serves the last known-good config.
