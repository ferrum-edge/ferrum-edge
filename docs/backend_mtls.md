...
- With `FERRUM_BACKEND_TLS_LIVE_RELOAD_ENABLED=true` (default), Ferrum watches backend TLS cert/key/CA/CRL sources, validates the active backend TLS configs on changed bytes, refreshes the backend CRL slot, clears HTTP-family backend client pools and the cached HTTP-family and `wss://` WebSocket backend TLS configs, and restarts active health checks. Existing in-flight backend requests and established WebSocket tunnels keep their current connections; new backend connections and WebSocket upgrades rebuild from the rotated material.
- Filesystem and provider work for backend pools, stream listeners, and active health checks uses the same process-wide bounded TLS source executor as every other TLS watcher. `FERRUM_TLS_SOURCE_MAX_BLOCKING_CONCURRENCY` limits concurrent blocking work and `FERRUM_TLS_SOURCE_LOAD_TIMEOUT_SECONDS` applies to admission plus resolution. A timeout keeps the last accepted backend config, listener, pools, and probe generation; it never tears down a working listener.
- Persistent source failures continue incrementing attempt/failure counters on every poll, but the bounded TLS event ring records transitions only: first fixed failure class, changed class, recovery, and later regression. Event source identities are digests and contain no source URI, credentials, or provider response.
...
- **Pool-per-cert-source**: Each unique combination of `backend_tls_client_cert_path`, `backend_tls_client_key_path`, and `backend_tls_server_ca_cert_path` produces a **separate connection pool entry** per protocol. Inline PEM values are represented by a SHA-256 source component instead of the raw PEM; PKCS#11 values use the URI selector as the non-secret pool key component:
  ...
  - **WebSocket (wss://)**: `rustls::ClientConfig` cached per TLS identity (no ALPN) and built on the bounded TLS source executor; the upgraded connection itself is not pooled. Built from the **target-effective** proxy — the CA, client cert/key, `verify_server_cert`, and SAN allow-list of the load-balancer-selected target's DestinationRule policy port, re-resolved before every connect attempt including retry rotations. A backend TLS **SNI override** is the one field this transport cannot apply: the WebSocket dial derives both the `Host` header and the TLS server name from the request URI, so an upgrade whose effective backend TLS carries an `sni` value is **refused before dialing** (`502`, non-retryable, no circuit-breaker or passive-health impact) rather than silently verifying the URI host. Configure the backend URI with the intended TLS server-name hostname (and use `dns_override` when that hostname must resolve to a specific address); a distinct SNI override is unsupported on direct WebSocket transport.
