...
A matched `mesh_route_dispatch` rule's total request deadline (`request_timeout_ms`) and per-attempt total bound (`attempt_timeout_ms`, Gateway API `timeouts.backendRequest`) are armed right after `before_proxy`, before target selection. A gRPC or gRPC-Web request folds both into its absolute RPC deadline, which the H3 gRPC dispatch paths and the bridge's gRPC-Web pass-through enforce; both bridges re-arm a fresh attempt budget for each retry (ending it before backoff), tell the backend each attempt's remaining budget in `grpc-timeout`, and charge an attempt-budget expiry after the request was sent to the backend with proxy core's `Backend deadline exceeded` terminal, exactly as proxy core does. The spent attempt budget ends when the expiry is charged, on every frontend, so a response plugin never reads it as the gateway's own deadline and rewrites the terminal as `Deadline exceeded at gateway` (#5744). On the bridge the charged terminal's `after_proxy` and response-committed plugins are bounded as over the gateway's own deadline terminal, with or without a remaining total deadline: a response-replacing plugin is skipped, other hooks get one poll, and pending work continues detached under the cleanup bound. An expiry while a buffered response body is read comes after `after_proxy` decorated the response head, so the terminal carries that head's gateway decorations (CORS, correlation, a gateway-minted affinity cookie) and `after_proxy` does not run again; only the response-committed plugins run over it. A `Plain` request is bounded on every H3 dispatch path at the same phases proxy core bounds HTTP/1.1 and HTTP/2 (`src/http3/route_deadline.rs`):
