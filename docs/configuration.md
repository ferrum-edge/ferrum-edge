...
| `FERRUM_MESH_CLUSTER_AUDIENCE` | No | _(unset)_ | Stable, operator-visible identifier this cluster is known by to its multi-cluster peers — the value a peer puts in `RemoteCluster.name` when it declares this cluster. Deliberately independent of any endpoint URL (an endpoint is mutable and must never be what a credential is bound to). **Not a secret**: it is an identifier, never a credential, and is safe to log. Control-plane side, a cross-cluster remote-discovery `MeshConfigSync.MeshSubscribe` request must present a JWT whose **single** `aud` is `ferrum-mesh-discovery:<this value>`; missing, malformed, multiple/ambiguous, and mismatched audiences are all refused. Unset means this control plane does not serve cross-cluster discovery and refuses every such subscription (**fail closed**), which is never a permissive posture. Ordinary local mesh and CP↔DP subscriptions are unaffected either way: they carry no audience, and every non-discovery gRPC surface (`ConfigSync`, xDS ADS, local `MeshSubscribe`) refuses a token that carries **any** `aud` — reporting a reserved `ferrum-mesh-discovery:` value distinctly from an unrelated one — so the two token purposes cannot be substituted for one another. Operator-minted CP↔DP tokens must therefore not stamp an `aud`. Refusals are counted as `ferrum_mesh_subscribe_audience_rejections_total{subscription,reason}` and audited as `audit.event="mesh_subscribe_audience_rejected"`; no token or claim value is logged. See `docs/mesh.md` "Remote-Discovery Audience Binding" |
