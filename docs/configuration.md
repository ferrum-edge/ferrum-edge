...
| `FERRUM_BACKEND_BLOCK_DANGEROUS_RANGES` | No | `true` | When `true` (default), backend egress to cloud-metadata/link-local (`169.254.0.0/16`, `fe80::/10`), the AWS IPv6 instance-metadata host (`fd00:ec2::254`), multicast (`224.0.0.0/4`, `ff00::/8`), and unspecified/this-host (`0.0.0.0/8`, `::`, `255.255.255.255`) is **blocked even under `both`**. Loopback and RFC1918/ULA stay allowed. Set `false` to restore fully-open egress (logs an unrestricted-egress warning) |
