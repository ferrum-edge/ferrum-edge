...
| `FERRUM_MAX_WEBSOCKET_FRAME_SIZE_BYTES` | No | `16777216` | Maximum WebSocket frame size in bytes; max message size = 4x frame size. Parser overflows of this global ceiling (or a tighter `ws_message_size_limiting` plugin ceiling) send RFC 6455 close code **1009** to both peers. End-to-end WebSocket Ping/Pong is **not** transparent: the gateway framer auto-answers Ping locally and does not forward Ping frames, so a single Ping yields exactly one Pong regardless of backend liveness |
