...
| `FERRUM_HTTP_HEADER_READ_TIMEOUT_SECONDS` | No | `10` | HTTP/1.1 header read timeout for proxy and admin listeners and for injector-mode (`FERRUM_MODE=injector`) admission-webhook connections; `0` disables it on all of them, leaving incomplete header blocks unbounded |
