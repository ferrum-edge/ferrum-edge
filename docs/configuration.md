...
| `FERRUM_K8S_WATCH_IDLE_RELIST_SECS` | No | `60` | Rebuild a watch scope's reflector from an authoritative Kubernetes list once that scope has delivered no event for this many seconds. Bounds how long a silently dead watch (a black-holed connection raises no error in kube-rs) can keep objects invisible to every reconcile. The replacement is swapped in make-before-break, so the scope never briefly reports zero objects. `0` disables recovery |
