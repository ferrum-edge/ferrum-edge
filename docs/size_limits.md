...
| Response body too large (unknown-length streaming, post-commit) | Committed backend status, then the body/stream terminates (`ResponseBodyTooLarge`); the gateway yields once before the error so the frontend flushes the head and the bytes accepted within the limit | No JSON `502` replacement after headers are committed |
