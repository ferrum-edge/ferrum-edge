...
| Response body too large (unknown-length streaming, post-commit) | Committed backend status, then the body/stream terminates (`ResponseBodyTooLarge`); the gateway yields once before the error so the frontend can flush the head and the bytes accepted within the limit (deterministic on HTTP/1.1, best effort on HTTP/2 and HTTP/3, where the flush runs on the connection task) | No JSON `502` replacement after headers are committed |
