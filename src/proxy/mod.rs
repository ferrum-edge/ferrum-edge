...
pub async fn handle_proxy_request(
    ...
    // Validate query parameter count. Uses a byte-scan count of '&' separators
    // instead of `.split('&').count()` to avoid iterator overhead. The parse
    // loop below is a separate pass but only runs after this validation succeeds.
    if state.max_query_params > 0 && !query_string.is_empty() {
        // Count separators + 1 = number of parameters. A single byte scan is
        // faster than the split iterator which must find boundaries and yield slices.
        let param_count = query_string
            .as_bytes()
            .iter()
            .filter(|&&b| b == b'&')
            .count()
