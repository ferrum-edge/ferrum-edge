...
async fn handle_tcp_connection(
    ...
    if let Some(tls_config) = frontend_tls_config {
        ...
        if proxy.backend_protocol == BackendProtocol::TcpTls {
            let backend_stream = connect_backend_tls(backend_addr, &backend_host, &proxy).await?;
...
/// A certificate verifier that accepts any certificate (for `backend_tls_verify_server_cert: false`).
#[derive(Debug)]
struct NoVerifier;
