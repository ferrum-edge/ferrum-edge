...
fn ai_transcript_audit_descriptor_is_active(
    config: &GatewayConfig,
    plugin_config: &PluginConfig,
) -> bool {
    if !plugin_config.enabled
        || plugin_config.plugin_name != "ai_transcript_audit"
        || plugin_config
            .config
            .get("grpc")
            .and_then(|grpc| grpc.get("descriptor_path"))
...
fn ai_transcript_audit_descriptor_preload_required_for_scope(
    config: &GatewayConfig,
    proxy_ids_to_rebuild: &HashSet<NamespacedResourceId>,
    rebuild_globals: bool,
) -> bool {
    config.plugin_configs.iter().any(|plugin_config| {
        ai_transcript_audit_descriptor_is_active(config, plugin_config)
            && match &plugin_config.scope {
                PluginScope::Global => rebuild_globals,
                PluginScope::Proxy => plugin_config.proxy_id.as_ref().is_some_and(|proxy_id| {
                    proxy_ids_to_rebuild.contains(&NamespacedResourceId::new(
...
impl PluginCache {
    ...
    /// Whether the exact delta-build scope reconstructs an active
    /// `ai_transcript_audit` with a node-local `grpc.descriptor_path`
    /// dependency.
    pub(crate) fn ai_transcript_audit_descriptor_preload_required(
        &self,
        config: &GatewayConfig,
        proxy_ids_to_rebuild: &HashSet<NamespacedResourceId>,
        rebuild_globals: bool,
