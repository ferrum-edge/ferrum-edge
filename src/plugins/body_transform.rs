...
mod tests {
    ...
    fn test_get_nested_value_simple() {
        ...
        assert_eq!(get_nested_value(&json, "name"), Some(&json!("Alice")));
    }

    #[test]
    fn test_get_nested_value_deep() {
        let json = json!({"user": {"address": {"city": "NYC"}}});
        assert_eq!(
            get_nested_value(&json, "user.address.city"),
            Some(&json!("NYC"))
        );
    }
    ...
    fn test_get_nested_value_missing() {
        ...
        assert_eq!(get_nested_value(&json, "user.age"), None);
        assert_eq!(get_nested_value(&json, "missing"), None);
    }

    #[test]
    fn test_set_nested_value_simple() {
        let mut json = json!({"name": "Alice"});
        assert!(set_nested_value(&mut json, "age", json!(30)));
        assert_eq!(json, json!({"name": "Alice", "age": 30}));
    }

    #[test]
    fn test_set_nested_value_deep() {
        let mut json = json!({"user": {"name": "Alice"}});
        assert!(set_nested_value(&mut json, "user.age", json!(30)));
    ...
    fn test_set_nested_value_creates_intermediates() {
        let mut json = json!({});
        assert!(set_nested_value(&mut json, "a.b.c", json!("deep")));
        assert_eq!(json, json!({"a": {"b": {"c": "deep"}}}));
    }

    #[test]
    fn test_remove_nested_value() {
        let mut json = json!({"user": {"name": "Alice", "age": 30}});
        let removed = remove_nested_value(&mut json, "user.age");
        assert_eq!(removed, Some(json!(30)));
        assert_eq!(json, json!({"user": {"name": "Alice"}}));
    }

    #[test]
    fn test_remove_nested_value_missing() {
        let mut json = json!({"user": {"name": "Alice"}});
        assert_eq!(remove_nested_value(&mut json, "user.missing"), None);
    }

    #[test]
    fn test_rename_nested_field() {
        let mut json = json!({"user": {"first_name": "Alice"}});
        assert!(rename_nested_field(
            &mut json,
            "user.first_name",
            "user.given_name"
        ));
        assert_eq!(json, json!({"user": {"given_name": "Alice"}}));
    }

    #[test]
    fn test_rename_across_levels() {
        let mut json = json!({"old": {"nested": {"key": "val"}}});
        assert!(rename_nested_field(&mut json, "old.nested.key", "flat_key"));
        assert_eq!(json, json!({"old": {"nested": {}}, "flat_key": "val"}));
    }
