...
///   `plugins::cors::canonicalize_exact_origin` admission); or canonicalizes to
///   a different string (scheme/host case, default port, IDNA, or normalized IP
///   spelling): the plugin would otherwise match the canonical browser Origin
///   even though Istio's literal exact matcher does not. Such policies defer
///   fail-closed instead of being broadened by projection.
...
/// a canonical `scheme://host[:port]` origin (the plugin's exact-origin
/// admission and serialization — see
/// `plugins::cors::canonicalize_exact_origin`). Envoy first probes every
/// matcher against the literal `*`; any exact/prefix/regex matcher that
...
/// credentialed CORS. A noncanonical exact, malformed/unknown origin matcher,
/// un-compilable `regex`, or invalid method/header token likewise makes the
/// policy non-translatable so it is left unprojected (deferred) rather than
/// silently approximated or failing `CorsPlugin` construction after
/// translation.
    ...
    if !cors_policy_translatable(cors) {
        tracing::warn!(
            ...
            "VirtualService http[].corsPolicy is not faithfully translatable (allowOrigins[] \
             ...
             canonical browser serialization, and a credentialed matcher with Envoy \
