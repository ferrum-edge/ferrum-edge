...
P_CONF_HTTP = 21050
P_CONF_ADMIN = 21051
P_PREC_HTTP_BE = 21052
P_PREC_TCP_BE = 21053
P_SHUT_HTTP_BE = 21054
P_SHUT_TCP_BE = 21055
P_FS_HTTP_BE = 21056
P_FS_TCP_BE = 21057
...
def phase_precedence(tmpdir: Path) -> None:
    ...
    spec_cli.write_text(spec_http_tcp(http_be=P_PREC_HTTP_BE, tcp_listen=21003, tcp_be=P_PREC_TCP_BE))
    ...
    http_be = HttpEcho(P_PREC_HTTP_BE); http_be.start(); http_be.ready.wait(3)
    tcp_be = TcpEcho(P_PREC_TCP_BE); tcp_be.start(); tcp_be.ready.wait(3)
    if not http_be.ready.is_set() or not tcp_be.ready.is_set():
        add(Row("P01", "P0", "CLI > env > conf for mode/spec/ports", "file",
                "conflicting -m/-c vs env vs ferrum.conf",
                "serves CLI spec on env ports; conf ports unused; database mode from env ignored",
                "blocked", write_ev("precedence-backend-bind.txt", "backend bind failed"),
                notes="harness backend ports busy"))
        http_be.stop(); tcp_be.stop()
        return
...
def phase_health_shutdown(tmpdir: Path) -> None:
    spec = tmpdir / "hs.yaml"
    spec.write_text(spec_http_tcp(http_be=P_SHUT_HTTP_BE, tcp_listen=21003, tcp_be=P_SHUT_TCP_BE))
    http_be = HttpEcho(P_SHUT_HTTP_BE); http_be.start(); http_be.ready.wait(3)
    tcp_be = TcpEcho(P_SHUT_TCP_BE); tcp_be.start(); tcp_be.ready.wait(3)
...
""")
    ...
    add(Row("R06", "P1", "reload --pid missing PID", "cli/reload", "reload --pid 999999",
            "exit != 0", "passed" if p.returncode != 0 else "failed", ev))
    # Isolated session: if #4079 regresses, only the setsid child is SIGHUP'd.
    p = run_cmd(["setsid", "--wait", str(BIN), "reload", "--pid", "0"], env=clean_env(), timeout=8)
    combined = f"{p.stdout}\n{p.stderr}"
    pid0_blob = (
        f"exit={p.returncode}\nstdout={p.stdout!r}\nstderr={p.stderr!r}\n"
        f"setsid_wait=1\n#4084 expect reject, not kill -HUP 0 (exit 129)\n"
    )
    ev = write_ev("reload-pid-zero-isolated.txt", pid0_blob)
    write_ev("reload-pid-zero.txt", pid0_blob)
    low = combined.lower()
    rejected = (
        p.returncode not in (0, 129)
        and (
            "pid 0" in low
            or "invalid --pid 0" in low
            or "not in 1.." in low
            or ("invalid value" in low and "0" in combined)
        )
    )
    add(Row("R07", "P1", "reload --pid 0 rejected (re-verify #4084)", "cli/reload",
            "setsid --wait ferrum-edge reload --pid 0",
            "exit != 0 and != 129; clap/range or unix_reload_pid_t rejects 0; no process-group SIGHUP",
            "passed" if rejected else "failed", ev,
            issue="https://github.com/ferrum-edge/ferrum-edge/issues/4079",
            notes=f"exit={p.returncode} #4084 on main should reject before kill"))
...
def phase_reload_fs(tmpdir: Path) -> None:
    ...
    spec = tmpdir / "fs.yaml"
    spec.write_text(spec_http_tcp(http_be=P_FS_HTTP_BE, tcp_listen=21063, tcp_be=P_FS_TCP_BE))
    http_be = HttpEcho(P_FS_HTTP_BE); http_be.start(); http_be.ready.wait(3)
    if not http_be.ready.is_set():
        add(Row("R10", "P1", "partial write rejected", "file/reload", "truncate in place",
                "last-good kept", "blocked", write_ev("reload-fs-backend-fail.txt", "http backend bind failed")))
        http_be.stop()
        return
...
def phase_docs_examples(tmpdir: Path) -> None:
    ...
    # tcp_udp_proxy.md File Mode examples (#4033 re-verify after merged #4066)
    tcpdoc = (ROOT / "docs/tcp_udp_proxy.md").read_text()
    enabled_in_doc = any(
        line.strip().startswith("enabled:")
        for line in tcpdoc.splitlines()
    )
    ...
    if "version:" not in block:
        block = 'version: "1"\n' + block
    if "plugin_configs:" not in block:
        block = block.rstrip() + "\nplugin_configs: []\nconsumers: []\nupstreams: []\n"
    ...
    p = run_cmd([str(BIN), "validate", "-m", "file", "-c", str(path)],
                env=gateway_env(path), timeout=25)
    ev = write_ev(
        "docs-tcp-udp.yaml.txt",
        f"enabled_field_in_doc={enabled_in_doc}\nexit={p.returncode}\n"
        f"{p.stdout}\n{p.stderr}\n---raw-spec-head---\n{raw_block[:500]}\n",
    )
    d03_ok = (not enabled_in_doc) and p.returncode == 0 and "Validation passed" in p.stdout
    add(Row("D03", "P0", "docs/tcp_udp_proxy.md File Mode examples (no enabled: true)", "docs/file",
            "confirm no enabled field; wrap version/plugin_configs; validate",
            "doc has no enabled:; wrapped copy-paste validates (re-verify #4033/#4066)",
            "passed" if d03_ok else "failed", ev,
            issue="https://github.com/ferrum-edge/ferrum-edge/issues/4033",
            notes=f"enabled_in_doc={enabled_in_doc} validate_exit={p.returncode}"))

    # explicit doc sentence + cleaned fragment (same wrappers)
    cleaned = "\n".join(ln for ln in raw_block.splitlines() if "enabled:" not in ln)
    ...
    p = run_cmd([str(BIN), "validate", "-m", "file", "-c", str(path)],
                env=gateway_env(path), timeout=25)
    explains = "no `enabled` field" in tcpdoc or "no enabled field" in tcpdoc
    ev = write_ev(
        "docs-tcp-udp-cleaned.yaml.txt",
        f"doc_explains_no_enabled={explains}\nexit={p.returncode}\n{p.stdout}\n{p.stderr}",
    )
    add(Row("D04", "P1", "tcp_udp_proxy.md documents missing enabled + wrapped examples validate", "docs/file",
            "doc prose + same YAML minus any enabled line",
            "prose says stream proxies have no enabled field; validate passes",
            "passed" if p.returncode == 0 and explains else "failed", ev))
...
""")
    ...
    add(Row("D07", "P1", "proxy-scoped plugin with proxy_id only validates", "docs/file",
            ...
            "validate accepts; docs now say proxy_id alone does not attach (#4038 hold after #4066)",
            "passed" if p.returncode == 0 else "failed", ev,
...
def main() -> int:
    ...
    wrapper = (ROOT / ".cargo/config.toml").read_text()
    sccache_configured = 'rustc-wrapper = "sccache"' in wrapper
    ev = write_ev(
        "sccache-wrapper.txt",
        f"sccache_on_path={shutil.which('sccache')!r}\n"
        f"configured_in_cargo={sccache_configured}\n"
        f"this_run_used=cargo --config 'build.rustc-wrapper=\"\"'\n",
    )
    add(Row("B03", "P1", "sccache rustc-wrapper documented fallback", "build",
            "inspect .cargo/config.toml + PATH",
            "wrapper may be missing; override documented and used",
            "passed" if sccache_configured else "failed", ev))
    cli = (ROOT / "src/cli.rs").read_text()
    has_pid_file = ("pid_file" in cli.lower()) or ("PID_FILE" in cli)
    ev = write_ev(
        "cli-no-pid-file.txt",
        f"pid_file_in_cli_rs={has_pid_file}\nreload uses --pid or pgrep\n",
    )
    add(Row("C15", "P2", "no PID-file support", "cli/reload", "inventory src/cli.rs",
            "--pid or pgrep only; no PID file",
            "passed" if not has_pid_file else "failed", ev))
