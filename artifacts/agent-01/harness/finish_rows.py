#!/usr/bin/env python3
"""Finish remaining rows. Do not invoke reload --pid 0 in-process."""
...
def rec(rows, row_id, pri, cap, expected, result, evidence, notes=""):
    rows.append(
        {
            "id": row_id,
            "priority": pri,
...
def main() -> None:
    ...
    rows = []
    tmp = Path(tempfile.mkdtemp(prefix="agent01d-"))
    http_be, proxy, admin = 21082, 21080, 21081
    echo = Echo(http_be)
    echo.start()
    time.sleep(0.2)
    spec = tmp / "r.yaml"
...
"""
    ...
    (EV / "reload-deleted.txt").write_text(f"echo={status} alive={alive}\n")
    rec(
        rows,
        "R08",
        "P1",
        ...
        "echo 200 + alive",
        "passed" if alive and status == 200 else "failed",
        "artifacts/agent-01/evidence/reload-deleted.txt",
        notes=f"echo={status} alive={alive}",
    )
    ...
    rec(
        ...
        notes=f"reload_exit={proc.returncode} rc={rc}",
    )

    write_spec()
    log_fh = (EV / "finish-fs.log").open("w")
    gw = subprocess.Popen([str(BIN), "run"], env=env, stdout=log_fh, stderr=subprocess.STDOUT)
    if not wait_port(admin):
...
"""
    ...
    rec(
        ...
        "R07",
        "P1",
        "reload --pid 0 is not rejected (kill process-group)",
        "exit != 0 with invalid-pid diagnostic",
        "failed",
        "artifacts/agent-01/evidence/reload-pid-zero-isolated.txt",
        notes="kill -HUP 0 signals the current process group; isolated run exited 129 (SIGHUP)",
    )

    (EV / "finish-rows.json").write_text(json.dumps(rows, indent=2) + "\n")
