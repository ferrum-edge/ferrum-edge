...
`alibaba-token-plan/qwen3.8-max` as implementation workers. Own task decomposition,
...
Do not recursively dispatch another Qwen3.8 Max, opencode, Grok, Sol, Opus, Fable, or Composer
...
- Model mismatch: stop the worker, record the exact diagnostic, correct the launch contract, and
  relaunch. Never claim `alibaba-token-plan/qwen3.8-max` without launch evidence.
