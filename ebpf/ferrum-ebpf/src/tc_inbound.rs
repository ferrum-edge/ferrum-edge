...
//! failed closed for NodeWaypoint except the relay's own marked datagrams from
//! a configured node address and
//! DNS responses from source port 53 to high pod-originated client ports
//! (>=32768). Listener-wide reply-source entries are deliberately not an
//! authorization proof here: a workload that can spoof that source tuple and
//! reuse the relay mark could otherwise inject into any enrolled pod. Service
//! reply sources require a future proof bound to the relay socket/session and
//! destination before they can be admitted. TCP semantics are unchanged.
...
fn guard_ipv4(ctx: &TcContext) -> Result<i32, i64> {
    ...
    match protocol {
        ...
        IPPROTO_UDP => {
            // A relay mark is reusable by the threat model, so it must be
            // paired with the configured-node source proof. A listener-wide
            // `(source address, source port)` map entry is not sufficient: it
            // is not bound to the relay socket, session, or destination pod.
...
fn guard_ipv6(ctx: &TcContext) -> Result<i32, i64> {
    ...
    match next_header {
        ...
        IPPROTO_UDP => {
            // IPv6 mirror of the v4 arm: only a configured node source plus
            // the relay mark is an authorization proof.
...
///
/// `source_authorized` is the configured-node source proof for both TCP and
/// UDP. In particular, a listener-wide UDP reply-source tuple is not accepted
/// because it can be replayed against an unrelated enrolled destination.
#[inline(always)]
