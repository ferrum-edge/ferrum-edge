...
- The `FERRUM_MESH_ALLOW_NO_CA` config-time presence check has a runtime complement (`enforce_mesh_inbound_fail_closed`, `src/modes/mesh`), at both the startup TLS-setup path and PeerAuthentication live reload: the mesh refuses an inbound mTLS/HBONE termination listener that would serve plaintext — resolved `ServerConfig` is `None` (PeerAuthentication DISABLE, or no usable server identity), or a configured gateway SVID verifier failed to load. **Refused under `FERRUM_MESH_PRODUCTION_MODE=true`** (a production mesh must serve mTLS); dev allows it with a loud warning — the no-identity *silent default* trap stays fail-closed at the config-time `FERRUM_MESH_ALLOW_NO_CA` gate, and an explicit DISABLE is intentional, so the runtime gate does not re-consult the opt-out. At reload, refusal rejects the slice and keeps the last-good mTLS config (fail-closed by retention). Gateway SVID material also backs the inbound server cert when no `FERRUM_FRONTEND_TLS_*` is set, so an SVID-only mesh serves mTLS rather than plaintext. `FERRUM_MESH_PRODUCTION_MODE` is read via `identity::production_mode()` (captured once at startup, threaded into the reload path); do not move these guardrail reads into `EnvConfig`.
