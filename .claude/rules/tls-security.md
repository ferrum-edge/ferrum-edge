...
- The `FERRUM_MESH_ALLOW_NO_CA` config-time presence check has a runtime complement (`enforce_mesh_inbound_fail_closed`, `src/modes/mesh`): at the startup TLS-setup path the mesh refuses to bring up an inbound mTLS/HBONE termination listener that would serve plaintext — resolved `ServerConfig` is `None` (PeerAuthentication DISABLE, or no usable server identity), or a configured gateway SVID verifier failed to load — unconditionally under `FERRUM_MESH_PRODUCTION_MODE=true`, else gated by `FERRUM_MESH_ALLOW_NO_CA` (dev opt-out warns and continues). Gateway SVID material also backs the inbound server cert when no `FERRUM_FRONTEND_TLS_*` is set, so an SVID-only mesh serves mTLS rather than plaintext. Keep the config-time and runtime checks in agreement — both read `identity::production_mode()` / `identity::allow_no_ca()`; do not move these reads into `EnvConfig`.
