...
`--effort` is accepted for CLI parity but ignored; Composer publishes no reasoning tiers, so never
claim an effort level was applied. Append `--fast` only when the user explicitly requests fast mode
for that dispatch or fleet.
