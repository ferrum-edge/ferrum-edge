...
use ferrum_edge::admin::AdminConnLimiter;
use ferrum_edge::admin::conn_limit::AdminConnRejectReason;
...
fn admits_up_to_cap_then_rejects() {
    ...
    // 4th is over the global cap. Use `.err()` so the unused `Ok` permit value
