...
#[test]
fn windowed_metrics_new_clamps_zero_window() {
...
#[tokio::test]
async fn monitor_zero_window_clamps_to_one_second() {
    let request_count = Arc::new(AtomicU64::new(0));
    let status_counts: Arc<DashMap<u16, AtomicU64>> = Arc::new(DashMap::new());
    status_counts.insert(200, AtomicU64::new(0));
    ...
    tokio::time::sleep(Duration::from_millis(50)).await;
    request_count.store(7, Ordering::Relaxed);
    status_counts.get(&200).unwrap().store(3, Ordering::Relaxed);

    tokio::time::sleep(Duration::from_millis(1200)).await;

    assert_eq!(windowed.window_seconds, 1);
    assert_eq!(windowed.requests_per_second.load(Ordering::Relaxed), 7);
    assert_eq!(
        windowed
            .status_codes_per_second
            .get(&200)
    ...
    shutdown_tx.send(true).unwrap();
    let result = tokio::time::timeout(Duration::from_secs(2), handle).await;
    assert!(result.is_ok(), "Monitor should shut down after zero clamp");
    assert!(
        result.unwrap().is_ok(),
        "Monitor should not panic after zero clamp"
    );
}
