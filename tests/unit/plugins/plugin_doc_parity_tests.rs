...
async fn protocol_matrix_matches_parity_meta_and_runtime_protocols() {
    ...
    for row in &rows {
        ...
        match meta.classification {
            ...
            _ if row.name == "ai_transcript_audit" => {
                // Documented capability envelope includes descriptor-based
                // native gRPC payload capture, which is opt-in through the
                // `grpc` block; the minimal config attaches HTTP only.
                assert!(
                    live.is_subset(&row.protocols),
                    "ai_transcript_audit live protocols {live:?} must be a subset of documented capability {:?}",
                    row.protocols
                );
                assert!(
                    live.contains(&ProxyProtocol::Http),
                    "ai_transcript_audit minimal config must remain HTTP-capable"
                );
            }
