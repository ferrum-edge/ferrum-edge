...
fn a_global_mcp_gateway_in_another_namespace_blocks_dedup() {
    ...
    // scoped associations and identity, not global applicability. Admission
    // must therefore refuse a foreign-namespace global `mcp_gateway` that the
    // runtime would still merge onto this proxy.
    let mut config = empty_config();
