...
fn test_http3_coalesce_min_clamped_above_max() {
    with_env_vars(
        &[
            ("FERRUM_MODE", "file"),
            ("FERRUM_FILE_CONFIG_PATH", "/path/config.yaml"),
            // Above the default MAX (32 KiB) — must clamp down to runtime MAX.
...
fn test_http3_coalesce_min_clamped_below_floor() {
    with_env_vars(
        &[
            ("FERRUM_MODE", "file"),
            ("FERRUM_FILE_CONFIG_PATH", "/path/config.yaml"),
            // Below the 1024-byte absolute floor — must clamp up.
...
fn test_http3_coalesce_max_clamped_above_cap() {
    with_env_vars(
        &[
            ("FERRUM_MODE", "file"),
            ("FERRUM_FILE_CONFIG_PATH", "/path/config.yaml"),
            // Above the 1 MiB absolute cap — must clamp down.
...
fn test_http3_flush_interval_floor() {
    with_env_vars(
        &[
            ("FERRUM_MODE", "file"),
            ("FERRUM_FILE_CONFIG_PATH", "/path/config.yaml"),
            // Below the 50 µs floor — must clamp up to prevent flush-every-poll.
...
fn test_http3_initial_mtu_below_min_rejected() {
    with_env_vars(
        &[
            ("FERRUM_MODE", "file"),
            ("FERRUM_FILE_CONFIG_PATH", "/path/config.yaml"),
            // 1199 is below QUIC's 1200-byte minimum — quinn rejects this, so we do too.
...
fn test_http3_initial_mtu_above_max_rejected() {
    with_env_vars(
        &[
            ("FERRUM_MODE", "file"),
            ("FERRUM_FILE_CONFIG_PATH", "/path/config.yaml"),
            // 65528 is one over quinn's 65527 maximum.
