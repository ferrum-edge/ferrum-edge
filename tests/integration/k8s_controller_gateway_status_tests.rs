...
async fn gateway_route_timeouts_reach_the_data_plane() {
    ...
    assert!(
        ...
        "the 500ms request deadline must answer before the 1.5s backend: {elapsed:?}"
    );
    ...
    assert!(
        ...
        "the 300ms backendRequest bound must fire before the 1.5s backend: {elapsed:?}"
    );
...
async fn gateway_route_request_timeout_spans_retry_attempts_and_backoff() {
    ...
    let harness = spawn_translated_route_gateway_with(&objects, Vec::new(), |config| {
        for proxy in &mut config.proxies {
            proxy.retry = Some(RetryConfig {
                ...
                backoff: BackoffStrategy::Fixed { delay_ms: 400 },
    ...
    // Attempts at ~0ms and ~400ms; the second backoff would end at ~800ms,
    // past the 700ms deadline, so the budget expires inside the backoff.
    let started = Instant::now();
    let response = client
    ...
    assert_eq!(body, r#"{"error":"Request timeout"}"#);
    assert!(
        elapsed >= Duration::from_millis(600) && elapsed < Duration::from_millis(1100),
        "the deadline must end the retry loop at ~700ms: {elapsed:?}"
    );

    // Without a route deadline the same retry policy runs to exhaustion: the
    // initial attempt plus three retries, ending with the backend's 503.
    let response = client
    ...
    let deadline_attempts = attempts("/deadline/thing");
    assert!(
        (1..=2).contains(&deadline_attempts),
        "the deadline must stop the retry planner early: {deadline_attempts} attempts"
    );
    assert_eq!(attempts("/retries/thing"), 4);
}
