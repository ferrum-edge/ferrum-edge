//! Functional HTTP/2 coverage for protocol body-size limit configuration.
//!
//! Run: `cargo test --test functional_tests -- --ignored functional_body_limit_h2 --nocapture`
...
#[ignore]
#[tokio::test]
async fn functional_body_limit_h2_request_body_zero_allows_above_default_body() {
    ...
    let req = Request::builder()
        ...
        .body(Full::new(Bytes::from(vec![b'a'; expected_len])))
    ...
    assert_eq!(
        ...
        "backend should receive the full H2 request body when request limit is disabled"
    );
...
#[ignore]
#[tokio::test]
async fn functional_body_limit_h2_response_body_zero_allows_above_default_body() {
    let backend_listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let backend_port = backend_listener.local_addr().unwrap().port();
    let expected_len = ABOVE_DEFAULT_BODY_LIMIT_BYTES;
    let (mut gateway, backend_task) = spawn_gateway(
    ...
    assert_eq!(
        body.len(),
        expected_len,
        "client should receive the full H2 response body when response limit is disabled"
    );
