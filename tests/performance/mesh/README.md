...
| `gateway_api_status_planning` | `plan_gateway_api_status_updates_budgeted` over 1k and 10k HTTPRoutes plus a 100×100 Gateway fanout case, with primary translation reuse built outside the measured iteration and the production 256 work budget | Guards #2397: status planning must stay bounded by the fair work window rather than O(routes × snapshot) per-object retranslation, per-Gateway full-snapshot rescans, or per-reference ReferenceGrant vector walks. Does not enforce a numeric threshold. |
