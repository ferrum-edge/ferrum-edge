...
/// un-compilable regexes, noncanonical literal exacts, and credentialed matchers
/// with Envoy allow-all semantics are left unprojected (deferred), not silently
/// approximated.
#[test]
fn vs_cors_policy_translated() {
    register_feature!(
        ...
        notes = "Translated to a proxy-scoped `cors` plugin (allowOrigins[] exact/prefix/regex StringMatch / legacy allowOrigin, allowMethods/allowHeaders/exposeHeaders/maxAge/allowCredentials/unmatchedPreflights). Omitted unmatchedPreflights preserves Istio FORWARD; exact origins must already use canonical browser serialization; uncredentialed matchers satisfying Envoy's literal-* probe preserve allow-all, while credentialed forms stay deferred instead of losing credentials.",
    );
